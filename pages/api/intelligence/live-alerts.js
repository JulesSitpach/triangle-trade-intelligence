...
async function generateLiveIntelligenceAlerts(userProfile) {
  ...
  const { data: recentSessions } = await supabase
    ...
    .select('data, stage_completed, created_at')  // stage_completed tracks page progression
    .eq('data->businessType', userProfile.businessType)
    .gte('created_at', new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString())
    .eq('stage_completed', 6)  // completed all 6 pages (foundation through alerts)
    .order('created_at', { ascending: false })
    .limit(10)

  if (recentSessions?.length > 0) {
    const avgSavings = recentSessions.reduce((acc, s) => {
      return acc + (s.data?.routingData?.potentialSavings || 0)
...
async function getNetworkActivity(userProfile) {
  ...
  const completedJourneys = networkSessions?.filter(s => s.stage_completed >= 6).length || 0  // completed full 6-page journey
