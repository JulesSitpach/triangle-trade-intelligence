...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (user && !isLoading) {
      console.log('User already logged in, redirecting...', user.isAdmin ? 'admin' : 'user');
      if (user.isAdmin) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error) {
        ...
        // Login successful - let the useEffect handle the redirect
        console.log('Login successful, user context will handle redirect');
        // Don't redirect here - let the useEffect above handle it when user state updates
      }
    } catch (err) {
