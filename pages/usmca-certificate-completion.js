...
export default function USMCACertificateCompletion() {
  ...
  const handlePreviewCertificate = async (authData) => {
    ...
    try {
      // Use AI-powered certificate generation (no separate endpoint needed)
      // Certificate data already generated by AI endpoint during workflow
  ...
  const generatePDFFromCertificate = async (professionalCertificate) => {
    ...
    yPos = 280;
    doc.text(`Generated by Triangle Trade Intelligence Platform with Trust Verification - ${new Date().toLocaleDateString()}`, 105, yPos, { align: 'center' });
