...
class SpecialistLeadEngine {
    ...
    try {
      ...
      const { data: sessions, error } = await supabase
        ...
        .select(`
          ...
          metadata
        `)
        .gte('stage_completed', 3) // At least completed Stage 3 (serious users)
  ...
  async processSessionIntoLead(session) {
    ...
    return {
      ...
      urgency,
      requirements,
      stageCompleted: session.stage_completed,
      ...
      metadata: {
        ...
        calculatorCompleted: session.stage_completed >= 5
      }
    }
  }
  ...
  calculateLeadScore(session, savingsData) {
    ...
    // Stage completion bonus (more stages = higher intent)
    score += Math.min(session.stage_completed * 8, 32) // Max 32 points for completing all stages
  ...
  calculateUrgency(session, savingsData) {
    const timeline = session.metadata?.priorities?.timeline
    const stageCompleted = session.stage_completed
    const totalSavings = savingsData.totalSavings
    
    if (timeline === 'immediate' || totalSavings > 15000000) return 'HIGH'
    if (timeline === 'within_3_months' && stageCompleted >= 7) return 'HIGH'
    if (timeline === 'within_6_months' || stageCompleted >= 5) return 'MEDIUM'
    return 'LOW'
  }
