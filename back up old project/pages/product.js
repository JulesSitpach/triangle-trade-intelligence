...
import { getIntelligentHSCodes, getIntelligenceStats } from '../lib/database-intelligence-bridge'
...
export default function ProductClassification() {
    ...
    try {
      ...
      const response = await fetch('/api/product-suggestions', {
        ...
        body: JSON.stringify({
          ...
          stage1Data: typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('triangle-foundation') || '{}') : {}
        })
      })
  ...
  const initializeStage2Data = async () => {
    if (typeof window !== 'undefined') {
      ...
      console.log('Raw stage1 localStorage:', stage1)
      console.log('Raw derived localStorage:', derived)

      if (stage1) {
        const stage1Parsed = JSON.parse(stage1)
        console.log('🔍 PARSED stage1 data:', stage1Parsed)
        console.log('🔍 Business type from localStorage:', stage1Parsed.businessType)
        setStage1Data(stage1Parsed)

        if (derived) {
          const derivedParsed = JSON.parse(derived)
          setDerivedData(derivedParsed)
          
          const enhanced = buildEnhancedContext(stage1Parsed, derivedParsed)
  ...
  const buildEnhancedContext = (stage1, derived) => {
    return {
      companyName: stage1.companyName,
      businessType: stage1.businessType,
      zipCode: stage1.zipCode,
      supplierCountry: stage1.primarySupplierCountry,
      importVolume: stage1.importVolume,
      timelinePriority: stage1.timelinePriority,
      derivedContext: {
        geographic: {
        ...
        business: {
          estimatedRevenue: estimateRevenue(stage1.businessType, stage1.importVolume),
          seasonalPattern: derived?.patterns?.seasonal || 'Unknown',
          experienceLevel: getExperienceLevel(stage1.importVolume),
          riskProfile: getRiskProfile(stage1.businessType, stage1.importVolume)
        },
        requirements: {
          specialHandling: derived?.patterns?.specialRequirements || [],
          complianceLevel: getComplianceLevel(stage1.businessType),
          temperatureControl: derived?.patterns?.specialRequirements?.includes('Temperature Control') || false,
          hazardousMaterials: checkHazardousMaterials(stage1.businessType)
        },
        triangleOpportunity: {
          confidence: derived?.shipping?.routeConfidence || 85,
          potentialSavings: calculatePotentialSavings(stage1.importVolume),
          recommendedRoute: derived?.shipping?.recommendedRoute || 'Unknown',
          implementationEase: getImplementationEase(stage1.businessType, stage1.importVolume)
        }
      }
    }
  }
  ...
  const suggestHSCodes = async (description, productIndex) => {
    if (!stage1Data || description.length < 4) return

    const businessType = stage1Data.businessType
  ...
  const selectHSCode = async (productIndex, hsCode, confidence) => {
    ...
    if (typeof hsCode === 'string') {
      ...
      const businessType = stage1Data?.businessType || 'Other'
    ...
    try {
      const learningData = {
        ...
        confidence: selectedConfidence,
        businessType: stage1Data?.businessType,
        companyName: stage1Data?.companyName,
  ...
  const runIntelligenceAnalysis = async () => {
    ...
    const businessType = stage1Data?.businessType || 'Manufacturing'
  ...
  useEffect(() => {
    if (products.length > 0 && products.some(p => p.hsCode)) {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('triangle-product', JSON.stringify(stage2Data))
      }
    }
  }, [products])
            ...
            {enhancedContext.derivedContext.requirements.specialHandling.length > 0 && (
                  ...
                  <div className="product-form">
                    <div className="form-group">
                      ...
                      <input
                        ...
                        placeholder={`${
                          stage1Data.businessType === 'Electronics' ? 'Smartphone accessories' :
                          stage1Data.businessType === 'Textiles' ? 'Cotton t-shirts' :
                          stage1Data.businessType === 'Medical' ? 'Medical devices' :
                      ...
                      {product.description && (() => {
                        const mismatchResult = detectBusinessTypeMismatch(product.description, stage1Data?.businessType)
                                ...
                                <div className="mismatch-content">
                                  ...
                                  <button 
                                    onClick={() => {
                                      ...
                                      const updatedStage1Data = { ...stage1Data, businessType: mismatchResult.detected }
                                      setStage1Data(updatedStage1Data)
                                      localStorage.setItem('triangle-foundation', JSON.stringify(updatedStage1Data))
                                      console.log(`🔧 Auto-fixed business type: ${stage1Data?.businessType} → ${mismatchResult.detected}`)
                            ...
                            >
                              ...
                              <div className="suggestion-source">
                                {suggestion.learnedFrom ? 
                                  ...
                                  suggestion.noMatch ? 
                                  `📊 Top ${stage1Data?.businessType || 'general'} codes` :
                                  '📊 Database match'
                                }
                              </div>
                            </button>
              ...
              {analysisComplete && products.filter(p => p.hsCode).length > 0 && (
                <div className="results-panel">
                  ...
                  <button onClick={proceedToStage3} className="bloomberg-btn bloomberg-btn-primary">
