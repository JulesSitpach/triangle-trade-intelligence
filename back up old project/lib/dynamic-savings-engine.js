...
export class DynamicSavingsEngine extends DynamicEngineBase {
  ...
  /**
   * Calculate dynamic annual savings based on multiple realistic factors
   */
  ...
  getCurrentTariffRates(businessType) {
    ...
    rates.bilateral += (Math.random() - 0.5) * volatility * 0.1
  ...
  getIndustryROIMultiplier(businessType) {
    ...
    return Math.max(0.7, baseMultipliers[businessType] || 1.0)
  }
  ...
  parseImportVolume(volumeString) {
    const volumeMap = {
      'Under $500K': 400000 + (Math.random() * 100000), // 300K-500K
      '$500K - $1M': 650000 + (Math.random() * 350000), // 650K-1M
      '$1M - $5M': 2200000 + (Math.random() * 2800000), // 2.2M-5M
      '$5M - $25M': 12000000 + (Math.random() * 13000000), // 12M-25M
      'Over $25M': 35000000 + (Math.random() * 65000000) // 35M-100M
    }
    
    return volumeMap[volumeString] || 1500000
  }
