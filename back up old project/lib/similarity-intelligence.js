  ...
  static async getFallbackSimilarityData(businessType) {
    try {
      ...
      const { data, error } = await supabase
        .from('workflow_sessions')
        .select('data, state')
        .like('data->stage_1->input->businessType', `%${businessType}%`)
        .limit(10)
      
      if (!error && data && data.length > 0) {
        console.log(`🔍 Found ${data.length} similar companies from database`)
        return data.map(session => ({
          business_type: session.data?.stage_1?.input?.businessType || businessType,
          ...
          import_volume: session.data?.stage_1?.input?.importVolume || '$1M - $5M'
        }))
      }
    } catch (error) {
