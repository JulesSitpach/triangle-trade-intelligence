...
export class CompoundIntelligenceTracker {
  ...
  static async assessSupplierCountryNovelty(supplierCountry) {
    const { count } = await supabase
      .from('workflow_sessions')
      .select('*', { count: 'exact' })
      .contains('stage_1', { primary_supplier_country: supplierCountry })
  ...
  static async assessVolumeNovelty(importVolume) {
    const { count } = await supabase
      .from('workflow_sessions') 
      .select('*', { count: 'exact' })
      .contains('stage_1', { import_volume: importVolume })
  ...
  static async assessTimelineNovelty(timelinePriority) {
    const { count } = await supabase
      .from('workflow_sessions')
      .select('*', { count: 'exact' })
      .contains('stage_1', { timeline_priority: timelinePriority })
