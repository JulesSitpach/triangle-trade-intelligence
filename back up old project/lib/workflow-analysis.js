...
// This helps optimize Stage 1 to only ask questions that drive outcomes
  ...
  try {
    ...
    if (!error && sessions) {
      ...
      const analysis = {
        ...
        // Pattern 4: Completion rates by stage
...
function analyzeSuccessPatterns(sessions) {
  // Sessions that reached Stage 8+ are "successful"
  const successfulSessions = sessions.filter(s => s.stage_completed >= 8)
...
function analyzeCompletionRates(sessions) {
  ...
  sessions.forEach(session => {
    if (session.stage_completed >= 1) stageReached.foundation++
    if (session.stage_completed >= 2) stageReached.product++
    if (session.stage_completed >= 3) stageReached.routing++
    if (session.stage_completed >= 8) stageReached.hindsight++
    if (session.stage_completed >= 9) stageReached.alerts++
  })
  
  return {
    foundation: Math.round((stageReached.foundation / sessions.length) * 100),
    product: Math.round((stageReached.product / sessions.length) * 100),
    routing: Math.round((stageReached.routing / sessions.length) * 100),
    hindsight: Math.round((stageReached.hindsight / sessions.length) * 100),
    alerts: Math.round((stageReached.alerts / sessions.length) * 100),
    dropoffAfterFoundation: Math.round(((stageReached.foundation - stageReached.product) / stageReached.foundation) * 100)
  }
}
