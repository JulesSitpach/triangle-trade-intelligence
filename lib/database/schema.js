...
const TABLES = {
  ...
  workflow_sessions: {
    columns: [
      ...
      'tariff_cache_strategy', 'substantial_transformation', 'manufacturing_process',
      'tax_id', 'company_address', 'company_country', 'contact_person',
      'contact_phone', 'contact_email', 'supplier_country'
    ],
    jsonbColumns: ['data', 'state', 'component_origins'],
    jsonbStructures: {
      // component_origins is an array of component objects with these fields
      component_origins: {
        fields: [
          'description', 'hs_code', 'origin_country', 'value_percentage',
          'confidence', 'mfn_rate', 'usmca_rate', 'section_301', 'section_232',
          'total_rate', 'savings_percentage', 'base_mfn_rate', 'rate_source',
          'data_source', 'stale', 'is_locked', 'ai_reasoning', 'hs_description',
          'hs_suggestions', 'alternative_codes', 'classification_source',
          'manufacturing_location', 'required_documentation', 'is_usmca_member',
          'enrichment_error'
        ],
        commonMistakes: {
          'confidence_score': 'Use "confidence" instead (components use "confidence", not "confidence_score")'
        }
      }
    },
    commonMistakes: {
 ...
 * @param {string} fieldName - Field being accessed within JSONB
 * @returns {object} { valid: boolean, error?: string }
 */
function validateJsonbField(tableName, columnName, fieldName) {
  const tableSchema = TABLES[tableName];
  if (!tableSchema) {
    return { valid: true }; // Table not in schema, skip validation
  }

  const jsonbStructure = tableSchema.jsonbStructures?.[columnName];
  if (!jsonbStructure) {
    return { valid: true }; // No structure defined for this JSONB column
  }

  // Check if field exists in schema
  if (jsonbStructure.fields && !jsonbStructure.fields.includes(fieldName)) {
    // Check for common mistakes
    if (jsonbStructure.commonMistakes?.[fieldName]) {
      return {
        valid: false,
        error: `Field "${fieldName}" does not exist in "${tableName}.${columnName}".\n${jsonbStructure.commonMistakes[fieldName]}`
      };
    }
    ...
    return {
      valid: false,
      error: `Field "${fieldName}" does not exist in "${tableName}.${columnName}".\nAvailable fields: ${jsonbStructure.fields.join(', ')}`
    };
  }
