...
export function getSupabaseClient() {
  if (!supabaseClient) {
    const supabaseUrl = SYSTEM_CONFIG.database.supabaseUrl;
    const supabaseKey = SYSTEM_CONFIG.database.anonKey;
...
export function getSupabaseServiceClient() {
  if (!serviceRoleClient) {
    const supabaseUrl = SYSTEM_CONFIG.database.supabaseUrl;
    const serviceKey = SYSTEM_CONFIG.database.serviceRoleKey;
...
export class DatabaseService {
  ...
  async getUSMCAQualificationRules(hsCode = null, productCategory = null) {
    let query = this.client.from(TABLE_CONFIG.usmcaRules).select('*');
  ...
  async getCountries(usmcaMembersOnly = false) {
    try {
      ...
      let query = this.client.from(TABLE_CONFIG.countries).select('*');
  ...
  async getTriangleRoutingOpportunities(originCountry = null, productCategory = null) {
    let query = this.client.from(TABLE_CONFIG.triangleRouting).select('*');
  ...
  async logClassification(productDescription, hsCode, confidence, method) {
    ...
    const { data, error } = await this.client
      .from(TABLE_CONFIG.classificationLogs)
      .insert(logEntry)
      .select();
  ...
  async logPerformance(operation, responseTime, success, errorMessage = null) {
    ...
    const { data, error } = await this.client
      .from(TABLE_CONFIG.performanceMetrics)
...
export const serverDatabaseService = new DatabaseService(true); // Server-side service
