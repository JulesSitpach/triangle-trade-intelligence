...
export class BatchQueryOptimizer {
  ...
  /**
   * Batch multiple similar queries into a single optimized query
   */
  static async batchQueries(queries, options = {}) {
    ...
    try {
      ...
      if (parallel && queries.length <= batchSize) {
        // Execute all queries in parallel (best for small batches)
        logInfo(`Executing ${queries.length} queries in parallel`)
        results = await Promise.all(queries.map(query => this.executeQuery(query)))
      } else {
        ...
        for (let i = 0; i < queries.length; i += batchSize) {
          ...
          const batchResults = await Promise.all(batch.map(query => this.executeQuery(query)))
