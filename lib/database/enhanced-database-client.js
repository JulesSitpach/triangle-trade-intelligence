 ...
 * Addresses core alignment issues: database connectivity & field mapping
 */
...
class EnhancedDatabaseClient {
  ...
  initializeClient() {
    ...
    this.client = createClient(supabaseUrl, supabaseKey, {
      ...
      global: {
        headers: {
          'X-Client-Info': 'enhanced-database-client'
        }
      }
    });
  }
    ...
    return {
      ...
      mapHSCode: (row) => {
        ...
        const mapped = {
          ...
          country_source: row.country_source || row.country || row.source_country,  // ✅ No hardcoded 'US' - let caller validate
  ...
  calculateConfidence(mappedData) {
    ...
    if (mappedData.country_source) score += 0.05;  // ✅ Bonus if country is specified (any country)
...
export default enhancedDatabaseClient;
