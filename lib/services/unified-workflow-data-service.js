 ...
 *
 * NOTE: This app uses custom JWT auth (triangle_session cookie), not Supabase auth.
 * Since RLS policies check auth.uid() which is always null with custom auth,
 * this query will fail with 401 on the frontend. That's expected and harmless -
...
async function hasDBProfile(userId) {
  ...
  try {
    ...
    if (error) {
      // Silently handle 401/403 errors (expected with custom auth + RLS)
    ...
    // Silently catch all errors - localStorage fallback will handle it
    return false;
  }
}
...
export async function getWorkflowData(userId = null) {
  ...
  // ✅ ARCHITECTURE FIX: Proper data flow separation
  // Frontend: Use localStorage only (no direct DB queries)
  // Backend API routes: Use service role key to query DB directly
  // Data flow: Frontend ↔ API routes ↔ Database
  //
  // This app uses custom JWT auth (not Supabase auth), so:
  // - RLS policies check auth.uid() which is always null on frontend
  // - Database queries on frontend would fail with 401
  // - Skip DB queries entirely on frontend to avoid console errors

  const isBrowser = typeof window !== 'undefined';

  if (isBrowser) {
    // Frontend: Use localStorage only
    const localData = getLocalStorageWorkflowData();
    if (localData) {
      console.log('[UnifiedDataService] ✅ Loaded workflow from localStorage (frontend)');
      return localData;
    }
    console.log('[UnifiedDataService] ⚠️ No workflow data found in localStorage');
    return null;
  } else {
    // Backend (server-side): Try database first
    if (userId && userId !== 'anonymous') {
      const dbData = await getDBWorkflowData(userId);
      if (dbData) {
        console.log('[UnifiedDataService] ✅ Loaded workflow from database (backend)');
        return dbData;
      }
    }
    console.log('[UnifiedDataService] ⚠️ No workflow data found in database');
    return null;
  }
}
