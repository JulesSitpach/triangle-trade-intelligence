...
export async function getUserSubscription(userId) {
  try {
    ...
    const { data: profile, error } = await supabase
      .from('user_profiles')
      .select('subscription_tier, trial_start_date, trial_end_date, subscription_start_date')
      .eq('user_id', userId)
      .single();
    ...
    return {
      ...
      usage_certificates: 0,
      period_start: profile.subscription_start_date || profile.trial_start_date || new Date().toISOString(),
      period_end: profile.trial_end_date || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString()
    };
  } catch (error) {
    await logDevIssue({
      type: 'api_error',
