...
class ResendAlertService {
   ...
   * 🔴 WEEK 1 ENHANCEMENT #6: Personalized subject line with user data
   */
  async sendCrisisAlert(alert, user) {
    try {
      console.log(`📧 Sending crisis alert email to ${user.email}${user.isVIP ? ' (VIP)' : ''}`);

      const emailHtml = this.generateAlertEmailHTML(alert, user);
      const emailSubject = this.generateSubject(alert, user); // ✅ Pass user for personalization
   ...
   * 🔴 LINKEDIN MEMBER ENHANCEMENT: Show financial impact in subject line
   * Format: "$103K Annual Impact: Section 301 Tariff on Your Microprocessors"
   */
  generateSubject(alert, user) {
    const emoji = alert.severity === 'critical' ? '🚨' :
                  alert.severity === 'high' ? '⚠️' : 'ℹ️';

    // Extract financial impact from alert data
    const financialImpact = this.extractFinancialImpact(alert);
    const impactPrefix = financialImpact ? `$${financialImpact} Impact: ` : '';

    // VIP treatment for $1M+ trade volume users
    const vipPrefix = user.isVIP ? '⭐ ' : '';

    // Personalized with contact person name (first name only)
    const firstName = user.contactPerson ? user.contactPerson.split(' ')[0] : '';
    const personalTouch = firstName ? `${firstName}, ` : '';

    // Build subject: [Emoji] [VIP] [Name] [$Impact] [Title]
    // Example: "🚨 ⭐ John, $103K Impact: Section 301 on Your Microprocessors"
    const maxTitleLength = 60 - impactPrefix.length - vipPrefix.length - personalTouch.length;
    const shortTitle = alert.title.substring(0, maxTitleLength) + (alert.title.length > maxTitleLength ? '...' : '');

    return `${emoji} ${vipPrefix}${personalTouch}${impactPrefix}${shortTitle}`;
  }

  /**
   * Extract financial impact from alert data for subject line
   * Returns human-readable format like "103K" or "1.2M"
   */
  extractFinancialImpact(alert) {
    // Check various fields where financial impact might be stored
    const impact = alert.estimated_annual_impact ||
                   alert.annual_cost_burden ||
                   alert.consolidated_impact?.total_annual_cost ||
                   alert.financial_impact ||
                   null;

    if (!impact || typeof impact !== 'number' || impact === 0) {
      return null;
    }

    // Format to human-readable (e.g., 103125 → "103K", 1250000 → "1.2M")
    if (impact >= 1000000) {
      return `${(impact / 1000000).toFixed(1)}M`;
    } else if (impact >= 1000) {
      return `${Math.round(impact / 1000)}K`;
    } else {
      return Math.round(impact).toString();
    }
  }

  /**
   * Get industry-specific context for subject line (🔴 WEEK 1 ENHANCEMENT #6)
   */
  getIndustryContext(alert, user) {
    const industry = user.industrySecor?.toLowerCase() || '';

    const industryPrefixes = {
      'automotive': 'Auto Trade Alert: ',
      'electronics': 'Tech Trade Alert: ',
      'textiles': 'Textile Alert: ',
      'agriculture': 'Ag Trade Alert: ',
      'energy': 'Energy Alert: ',
      'chemicals': 'Chemical Trade Alert: '
    };

    for (const [key, prefix] of Object.entries(industryPrefixes)) {
      if (industry.includes(key)) {
        return prefix;
      }
    }

    return 'Trade Alert: '; // Default
  }

  /**
   * Generate beautiful HTML email
   * 🔴 WEEK 1 ENHANCEMENT #6: Personalized greeting with contact_person and VIP treatment
   */
  generateAlertEmailHTML(alert, user) {
    const severityColor = alert.severity === 'critical' ? '#dc2626' :
                         alert.severity === 'high' ? '#f59e0b' : '#3b82f6';

    // Personalized greeting with contact_person
    const firstName = user.contactPerson ? user.contactPerson.split(' ')[0] : null;
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f9fafb;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background-color: #ffffff; padding: 30px; border-radius: 12px 12px 0 0; border-bottom: 4px solid ${severityColor};">
      <div style="font-size: 24px; font-weight: bold; color: #111827; margin-bottom: 8px;">
        Triangle Trade Intelligence${user.isVIP ? ' ⭐' : ''}
      </div>
      <div style="font-size: 14px; color: #6b7280;">
        Real-Time Government Alert${user.isVIP ? ' - VIP Priority' : ''}
      </div>
    </div>

    <!-- VIP Badge (for $1M+ trade volume users) -->
    ${user.isVIP ? `
    <div style="background-color: #fef3c7; padding: 12px 30px; border-left: 4px solid #f59e0b;">
      <div style="font-size: 13px; font-weight: 600; color: #92400e;">
        ⭐ VIP Priority Alert - Your $${(user.annualTradeVolume / 1000000).toFixed(1)}M trade volume qualifies you for priority support
      </div>
    </div>
    ` : ''}
    ...
    <div style="background-color: #ffffff; padding: 30px;">
      ...
      <!-- Personalized Greeting (🔴 WEEK 1 ENHANCEMENT #6) -->
      <div style="font-size: 15px; color: #4b5563; margin-bottom: 16px;">
        ${greeting}
      </div>
  ...
  async getAffectedUsers(alert, supabase) {
    try {
      ...
      console.log(`📧 Alert "${alert.title}" (${alert.severity}): ${filteredUsers.length} users qualified`);
