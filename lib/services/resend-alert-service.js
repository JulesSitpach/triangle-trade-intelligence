...
class ResendAlertService {
   ...
   * 🔴 WEEK 1 ENHANCEMENT #6: Uses contact_person, VIP treatment, industry-specific subjects
   */
  generateSubject(alert, user) {
    ...
    const vipPrefix = user.isVIP ? '⭐ VIP Alert: ' : '';

    // Industry-specific subject lines
    const industryContext = this.getIndustryContext(alert, user);
    ...
    return `${emoji} ${vipPrefix}${personalTouch} ${industryContext}${alert.title.substring(0, 40)}${alert.title.length > 40 ? '...' : ''}`;
  }
