...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f9fafb;">
  ...
  async getAffectedUsers(alert, supabase) {
    try {
      // Query users who should receive this alert
      const { data: users, error } = await supabase
        .from('user_profiles')
        .select('id, email, full_name, company_name')
        .eq('email_notifications', true)
        .in('subscription_tier', ['Professional', 'Premium', 'Enterprise']);
      ...
      // Filter users based on alert criteria
      // In production, you'd match HS codes, business types, etc.
      return users || [];
