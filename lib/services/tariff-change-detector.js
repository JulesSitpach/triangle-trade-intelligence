...
import rateValidationAgent from '../agents/rate-validation-agent.js';
          ...
          if (policyData && policyData.is_policy_announcement) {
            ...
            if (policyData.rate_changes && policyData.rate_changes.length > 0) {
              for (const change of policyData.rate_changes) {
                try {
                  // Log rate change to database
                  const { data: loggedChange, error: logError } = await this.supabase
                  ...
                  // 5. 🆕 EVENT-DRIVEN VALIDATION (Nov 21, 2025)
                  // Immediately validate rate change with RateValidationAgent
                  console.log(`  🔍 Triggering validation for ${change.hs_code}...`);
                  ...
                  const validation = await rateValidationAgent.validateRate({
                    ...
                    change_id: loggedChange.id
                  });

                  // Update policy_tariffs_cache if validation passed
                  if (validation.should_update_cache && validation.confidence >= 0.80) {
                    ...
                    console.log(`  ✅ Cache updated: ${change.hs_code} = ${(validation.validated_rate * 100).toFixed(1)}% (confidence: ${(validation.confidence * 100).toFixed(0)}%)`);
                  } else if (validation.confidence < 0.80) {
                    console.log(`  ⚠️ Low confidence (${(validation.confidence * 100).toFixed(0)}%), flagged for manual review`);
                  }

                  // Mark change as validated in tariff_changes_log
                  await this.supabase
                    .from('tariff_changes_log')
                    .update({
                      is_validated: true,
                    ...
                    .eq('id', loggedChange.id);

                } catch (logError) {
                  console.warn(`  ⚠️ Failed to log/validate rate change for ${change.hs_code}:`, logError.message);
                }
              }
            }
...
`;
  ...
  /**
   * Update policy_tariffs_cache with validated rate
   * Called after RateValidationAgent confirms rate change
   */
  async updatePolicyCacheWithValidation(hs_code, validation) {
    try {
      const { validated_rate, confidence, validation_status } = validation;

      // Upsert to policy_tariffs_cache
      const { error } = await this.supabase
        .from('policy_tariffs_cache')
        .upsert({
          hs_code,
          section_301: validated_rate, // Assume Section 301 for now (most common)
          section_232: 0, // Reset Section 232 (would need separate detection)
          last_updated: new Date().toISOString(),
          confidence: confidence,
          validation_status: validation_status,
          source: 'rss_validation_agent'
        }, {
          onConflict: 'hs_code'
        });
      ...
      console.log(`[TariffChangeDetector]   💾 Updated policy cache: ${hs_code} = ${(validated_rate * 100).toFixed(1)}%`);

    } catch (error) {
      console.error(`[TariffChangeDetector]   ❌ Failed to update policy cache:`, error.message);
    }
  }
