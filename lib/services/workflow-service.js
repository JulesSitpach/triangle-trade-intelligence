...
class WorkflowService {
  ...
  async processCompleteWorkflow(formData) {
    ...
    try {
      ...
      // Skip trust microservices for now - go directly to stable database-driven API
      logInfo('Using database-driven API directly (trust services bypassed)');
      let response = await fetch(`${this.baseURL}/api/database-driven-usmca-compliance`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      if (!response.ok && response.status === 404) {
        logInfo('Database-driven API unavailable, falling back to simple API');
        response = await fetch(`${this.baseURL}/api/simple-usmca-compliance`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            action: 'complete_workflow',
  ...
  validateFormData(formData) {
    ...
    if (!formData.component_origins || formData.component_origins.length === 0) {
      ...
      formData.component_origins.forEach((comp, index) => {
        ...
        if (!comp.hs_code) {
          errors.push(`Component ${index + 1} requires an HS code`);
        }
      });
    }
  ...
  getCountryCode(countryName) {
    const codes = {
      ...
      'South Korea': 'KR', 'Thailand': 'TH', 'Malaysia': 'MY', 'Indonesia': 'ID'
    };
