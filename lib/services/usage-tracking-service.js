...
export async function getUserUsageStats(userId) {
  try {
    ...
    return {
      ...
      analysesCount: data.analysis_count,
...
export async function getUserUsageHistory(userId, months = 6) {
  try {
    ...
    return data.map(record => ({
      ...
      analysesCount: record.analysis_count,
...
export async function getAggregateUsageStats() {
  try {
    ...
    const { data, error } = await supabase
      .from('monthly_usage_tracking')
      .select('subscription_tier, analysis_count')
      .eq('billing_period_start', periodStartStr);
    ...
    const stats = data.reduce((acc, record) => {
      ...
      acc[tier].totalAnalyses += record.analysis_count;
      return acc;
    }, {});
    ...
    return {
      ...
      totalAnalyses: data.reduce((sum, r) => sum + r.analysis_count, 0)
    };
  } catch (error) {
