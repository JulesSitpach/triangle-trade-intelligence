...
class RSSPollingEngine {
  ...
  async processRSSItems(feed, items, useAI = false) {
    ...
    for (const item of items) {
      try {
        ...
        // ✅ VALIDATION: Skip items where content extraction failed
        ...
        if (crisisDetection.hasCrisisKeywords || feed.priority_level === 'high') {
          ...
          // ✅ FINAL VALIDATION: Double-check content exists before database insert
          if (!content.description && !content.fullContent) {
            console.error(`❌ CRITICAL: Attempting to insert empty content to database:`, {
              feedName: feed.name,
              title: item.title,
              guid: item.guid || item.link
            });
            continue; // Skip - never insert empty content
          }
          ...
          const { data: newActivity, error: insertError } = await this.supabase
            ...
            .insert({
              ...
              content: content.fullContent || null, // Use NULL instead of empty string
              ...
              status: content.description || content.fullContent ? 'success' : 'incomplete' // ✅ Mark incomplete if no content
            })
            .select()
            .single();
   ...
   * ✅ VALIDATE: Return error flag if ALL content fields are empty
   */
  extractContent(item) {
    ...
    // ✅ VALIDATION: Check if ANY actual content was extracted
    if (!cleanDescription && !cleanContent) {
      console.warn(`⚠️ extractContent() failed - all fields empty for item:`, {
        title: item.title?.substring(0, 50),
  ...
  async analyzeRSSItemWithAI(title, description) {
    try {
      const prompt = `You are a STRICT trade policy analyst for Triangle Trade Intelligence. Only alert for actual tariff/trade policy changes.

OUR VALUE PROPOSITION:
- Mexico triangle routing (Canada→Mexico→US) as USMCA advantage
- AI-powered USMCA certificate generation and compliance
- Target users: North American importers/exporters in electronics, automotive, textiles, manufacturing

RSS ITEM TO ANALYZE:
Title: ${title}
Description: ${description}

STRICT SCORING (ONLY 5+ creates alert):
5-10: ONLY for actual trade/tariff policy changes
0-4: Everything else (reject 99% of non-trade content)

MUST SCORE 5+ (create alert) ONLY IF about:
- Tariff rate changes (specific %, specific countries/products)
- USMCA/CUSMA/T-MEC updates (regional content, rules of origin)
- Section 301 or Section 232 actions (China, Vietnam, steel, aluminum)
- Trade agreement negotiations (active, with specific countries)
- Import/export compliance requirements (specific rules changing)
- Labor standards for manufacturing (affects USMCA qualification)
- Country-specific trade policy (China, Mexico, Canada, US tariffs/duties)

MUST SCORE 0-4 (REJECT - don't create alert) for:
- Postal service updates/products ❌
- Presidential/disaster declarations ❌
- Healthcare programs (TRICARE, Medicare, etc.) ❌
- Advisory committee meetings ❌
- Patent application procedures ❌
- Housing programs (HOME, HUD, etc.) ❌
- General government regulations (not trade-related) ❌
- Stock market/earnings reports ❌
- Economic indicators (GDP growth, inflation general) ❌
- Opinion pieces ❌
- Historical events ❌
- Wage/labor laws (not manufacturing trade-related) ❌

CRITICAL REJECTION KEYWORDS - If present, ALWAYS score 0:
postal, disaster, healthcare, TRICARE, Medicare, patent, housing, home program, committee meeting, advisory board

EXAMPLES OF CORRECT SCORING:
✅ "US increases Section 301 tariffs on Chinese semiconductors to 60%" → Score 9 (ALERT)
✅ "USMCA regional content threshold for automotive increased to 75%" → Score 8 (ALERT)
✅ "Mexico increases labor standards for manufacturing" → Score 7 (ALERT)
✅ "Tariff on Vietnamese steel imports raised from 15% to 25%" → Score 8 (ALERT)

❌ "New postal products launched by USPS" → Score 0 (REJECT)
❌ "Presidential declares disaster area" → Score 0 (REJECT)
❌ "TRICARE health plan changes benefits" → Score 0 (REJECT)
❌ "Advisory committee meets to discuss guidelines" → Score 0 (REJECT)
❌ "Patent application requirements updated" → Score 0 (REJECT)
❌ "HOME housing program announces new rules" → Score 0 (REJECT)
❌ "China GDP growth slows to 4%" → Score 1 (general news, not policy)
❌ "Tesla stock drops on earnings miss" → Score 1 (not trade policy)
...
Return ONLY valid JSON:
{
  ...
  "reasoning": "Specific 60% Section 301 tariff increase on semiconductors directly impacts sourcing decisions"
}`;
  ...
  async generateCrisisAlert(feed, activity, crisisDetection) {
    try {
      ...
      // Extract affected industries from crisis detection if available
      const affectedIndustries = crisisDetection.affectedIndustries || [];

      // Create crisis alert - FIXED SCHEMA ALIGNMENT
      const { data: alert, error: insertError } = await this.supabase
        ...
        .insert({
          ...
          severity: severity,  // ✅ FIXED: was severity_level (column doesn't exist)
          is_active: true,
          ...
          relevant_industries: affectedIndustries.length > 0 ? affectedIndustries : [],
          confidence_score: Math.min(crisisDetection.score / 10, 1.0),  // ✅ Convert 0-10 scale to 0-1 decimal
          detection_source: 'rss_feed',  // ✅ ADDED: Track that this came from RSS feed
          source_url: activity.link  // ✅ ADDED: Save original RSS link for reference
          // REMOVED: source_type, source_id, requires_action (columns don't exist in schema)
        })
        .select()
        .single();
