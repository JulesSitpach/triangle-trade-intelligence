...
class USITCTariffSync {
   ...
   * Uses correct POST endpoint with query payloads (usitcDataWebAPI)
   */
  async fetchUSITCRates(hsCode) {
    try {
      logInfo(`🔍 [USITC-SYNC] Fetching rates for ${hsCode}`);

      const result = await usitcDataWebAPI.verifyAndGetTariffRates(hsCode);

      if (result) {
        logInfo(`✅ [USITC-SYNC] Got rates for ${hsCode}: MFN ${(result.mfn_rate * 100).toFixed(1)}%`);
      ...
      logInfo(`⚠️ [USITC-SYNC] No data returned for ${hsCode}`);
      return null;

    } catch (error) {
      // USITC API down (503) or other error - return null and let it try next code
