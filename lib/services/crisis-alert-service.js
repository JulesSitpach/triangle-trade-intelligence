...
export class CrisisAlertService {
  ...
  async getAffectedUsers(crisisAnalysis) {
    try {
      ...
      if (!workflows || workflows.length === 0) {
        console.log('No workflow data found - using sample users for crisis alerts');
        return this.getSampleAffectedUsers(crisisAnalysis);
      }
  ...
  /**
   * Get sample affected users (fallback when no workflow data exists)
   * 🔴 WEEK 1 ENHANCEMENT #5: Includes VIP flag and contact_person
   */
  getSampleAffectedUsers(crisisAnalysis) {
    const sampleUsers = [
      {
        id: 'sample-electronics-001',
        companyName: 'Sample Electronics Inc',
        email: 'demo@example.com',
        contactPerson: 'Demo User',
        annualTradeVolume: 2500000,
        industrySecor: 'Electronics',
        componentOrigins: [{ origin_country: 'CN', percentage: 60 }],
        subscriptionTier: 'Professional',
        isVIP: true
      }
    ];

    // Filter sample users by crisis categories
    return sampleUsers.filter(user => {
      if (crisisAnalysis.affectedCategories.length === 0) return true;
      return crisisAnalysis.affectedCategories.some(cat =>
        user.industrySecor?.toLowerCase().includes(cat.name.replace('_', ' '))
      );
    });
  }
}
