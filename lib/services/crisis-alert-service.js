...
export class CrisisAlertService {
  ...
  async getAffectedUsers(crisisAnalysis) {
    try {
      // TODO: Query database for real users once user_profiles table has trade data
      ...
      await logDevIssue({
        ...
        message: 'Using sample user data instead of database query for crisis alerts',
        data: {
          crisisLevel: crisisAnalysis.crisisLevel,
          affectedCategories: crisisAnalysis.affectedCategories.map(c => c.name),
          reason: 'user_profiles table lacks trade profile data (HS codes, categories)'
        }
      });
