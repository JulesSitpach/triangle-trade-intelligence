...
class WorkflowDataCaptureService {
  ...
  async captureCompanyInfo(data) {
    // ✅ Validate required fields before capturing company info
    if (!data.company_name && !data.exporter_name) {
      throw new Error('company_name is required in captureCompanyInfo. Expected: company legal name');
    }
    if (!data.country && !data.company_country && !data.headquarters_country) {
      throw new Error('country is required in captureCompanyInfo. Expected: headquarters country (US, CA, MX, CN, etc.)');
    }
    ...
    const companyData = {
      ...
      // Location data
      headquarters_country: data.country || data.company_country,  // ✅ No hardcoded 'US' - use validated value
    ...
    try {
      ...
      const { data: completion, error: completionError } = await supabase
        ...
        .insert({
          ...
          certificate_id: this.workflowData.certificate?.certificate_number,
