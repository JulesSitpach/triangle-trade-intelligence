...
class AlertImpactAnalysisService {
  ...
  buildAdditivePrompt({ existingAnalysis, consolidatedAlerts, affectedComponents, countryConfig, userProfile, monthsToReview }) {
    ...
    const annualTradeVolume = userProfile.tradeVolume || userProfile.annual_trade_volume || 0;
    const portfolioImpact = this.calculatePortfolioImpact(affectedComponents, annualTradeVolume);
