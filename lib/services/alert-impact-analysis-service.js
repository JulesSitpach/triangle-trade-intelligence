...
6. FROM YOUR TRADE ADVISOR (Broker tone - REQUIRED):
   - Write like a senior customs broker or trade compliance officer advising them
   - Provide CBP compliance guidance specific to their situation
   - Reference regulatory risk based on THEIR components, origins, RVC status
   - Name specific compliance concerns (e.g., "integrated circuits face heightened CBP scrutiny")
   - Provide actionable broker recommendations (customs broker engagement, documentation strategy)
   - Reference what "strengthens your audit defensibility" based on their specific sourcing
   - Technical notes: any origin rules, documentation requirements, supplier validation needed
   - Make it clear this is professional advisory (not legal advice) but from a customs/trade expert
   - Reference how their current situation (e.g., "${userProfile.regionalContent}% RVC") affects compliance risk
...
RESPONSE FORMAT (JSON - MUST provide specific details, NOT placeholders):
{
  ...
  "from_your_trade_advisor": {
    "situation_assessment": "Professional customs broker assessment of their compliance risk: reference their specific situation (${userProfile.regionalContent}% RVC, ${userProfile.companyName}, ${userProfile.industry_sector || userProfile.businessType} sector)",
    "cbp_compliance_guidance": "Specific CBP concern based on THEIR components/origins (e.g., 'Integrated circuits from China face heightened CBP scrutiny for origin verification')",
    "audit_defensibility": "How their current sourcing (or proposed changes) would 'strengthen your audit defensibility' - be specific (e.g., 'Mexico-origin documentation is more straightforward to verify than China sourcing')",
    "supplier_validation": "Technical note: What origin/qualification documentation they need from suppliers - SPECIFIC to their components/countries",
    "professional_advisory": "1-2 sentences positioning this as expert trade advisory (not legal advice): 'From a customs compliance perspective, [their company] presents [specific risk level] risk because [reason based on their RVC/components]'"
  }
}`;
  }
