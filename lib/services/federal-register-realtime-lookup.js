...
class FederalRegisterRealtimeLookup {
  ...
  async _searchFederalRegisterForHS(normalizedHS, originalHS) {
    try {
      // Federal Register API advanced search
      // Filter by USTR agency and document type (NOTICE for Section 301)
      const searchTerm = originalHS || normalizedHS;
      ...
      params.append('conditions[type][]', 'NOTICE'); // Section 301 notices
  ...
  async _extractRateFromDocument(doc, hsCode) {
    try {
      ...
      // Fetch raw text from URL (Federal Register stores text separately)
      let fullText = '';
      if (fullDoc.raw_text_url) {
        const textResponse = await fetch(fullDoc.raw_text_url);
        if (textResponse.ok) {
          fullText = await textResponse.text();
        }
      }

      // Fallback to abstract if no raw text
      if (!fullText || fullText.length < 100) {
        fullText = fullDoc.abstract || '';
      }
