...
export class CascadedTariffService {
  ...
  formatTariffResult(record, matchType) {
    return {
      ...
      country: record.country || null  // ✅ No hardcoded 'US' - let caller provide destination country context
    };
  }
  ...
  getRealisticFallbackRates(hsCode, searchContext = {}) {
    if (!hsCode || hsCode.length < 2) {
      return { mfn_rate: SYSTEM_CONFIG.business.emergencyFallbackRate, usmca_rate: 0.0 }; // Configurable fallback
    }
    ...
    const chapterRates = {
      ...
      // Default fallback rates - use configurable emergency rate
      default: { 
        mfn_rate: SYSTEM_CONFIG.business.emergencyFallbackRate, 
        usmca_rate: 0.0 
      } // General manufacturing average from config
    };
