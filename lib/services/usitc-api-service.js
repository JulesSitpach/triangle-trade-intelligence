...
export class USITCApiService {
  ...
  static async getTariffRates(hsCode, destinationCountry = 'US') {
    ...
    try {
      ...
      // USITC API endpoint for HTS tariff schedule
      // Docs: https://dataweb.usitc.gov/api/hts
      const url = `${USITC_API_BASE}/hts/search`;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          hts_number: normalizedHS,
          include_rates: true,
          year: new Date().getFullYear() // Current year rates
        })
      });
      ...
      // Parse USITC response
      const result = data.results[0]; // First match
      ...
      const tariffData = {
        ...
        mfn_rate: this.parseRate(result.rates?.general || result.rates?.column1),
        mfn_text_rate: result.rates?.general_text || result.rates?.column1_text || 'Unknown',

        // USMCA/NAFTA preferential rate
        usmca_rate: this.parseRate(result.rates?.special || 0),

        // Section 301/232 - USITC may not have these (they're in Federal Register)
        section_301: 0, // Will need Federal Register API for this
        section_232: 0, // Will need Federal Register API for this

        // Metadata
        rate_source: 'usitc_api',
        data_source: 'usitc_dataweb',
        last_verified: new Date().toISOString(),
        api_response: result // Store full response for debugging
      };
