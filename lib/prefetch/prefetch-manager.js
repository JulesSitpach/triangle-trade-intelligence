...
import { logInfo, logError, logPerformance, logAPICall } from '../utils/production-logger.js'
...
export class PrefetchManager {
  ...
  constructor() {
    ...
    try {
      ...
      logInfo('PrefetchManager initialized successfully', { 
        enabled: this.enabled,
        rateLimitRequests: this.rateLimiter.maxRequests,
        rateLimitWindow: this.rateLimiter.timeWindow 
      })
    } catch (error) {
      logError('PrefetchManager initialization failed', { error: error.message })
      this.initialized = false
      this.enabled = false
      // Create minimal fallback instance
      this.cache = new Map()
      this.metrics = { requests: 0, hits: 0, misses: 0, errors: 0, totalLatency: 0 }
      PrefetchManager.instance = this
    }
  }
