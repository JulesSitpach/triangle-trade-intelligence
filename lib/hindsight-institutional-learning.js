 ...
 * Hindsight: "If I knew then what I know now" - Extract patterns for future users
...
export class HindsightInstitutionalLearning {
  ...
  static async getSimilarCompletedJourneys(userData) {
    try {
      ...
      const { data: similarJourneys } = await supabase
        ...
        .not('alerts', 'is', null)  // Completed full journey
        .limit(20)
  ...
  static calculateAverageOutcome(journeys, metric) {
    ...
    const values = journeys.map(j => {
      try {
        if (metric === 'savings_percentage') return j.alerts?.savings_percentage || 25
        if (metric === 'total_savings') return j.alerts?.total_savings || 1000000
        if (metric === 'implementation_time') return j.alerts?.implementation_time || 6
        return 0
      } catch {
        return 0
      }
  ...
  static calculateSuccessRate(journeys) {
    ...
    const successful = journeys.filter(j => j.alerts?.success_status === 'completed').length
  ...
  static calculatePeerRanking(userData, journeyResults, similarJourneys) {
    ...
    const betterPerformers = similarJourneys.filter(j => {
      try {
        return (j.alerts?.total_savings || 0) > userSavings
      } catch { return false }
    }).length
  ...
  static identifyOptimalRoute(userData, similarJourneys) {
    ...
    const routes = similarJourneys.map(j => j.routing?.optimal_route || 'Mexico').filter(Boolean)
  ...
  static calculateAverageTimeline(similarJourneys) {
    const timelines = similarJourneys.map(j => j.alerts?.implementation_time || 6).filter(t => t > 0)
