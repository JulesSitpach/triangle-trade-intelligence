...
export class UnifiedHSClassifier {
        ...
        const matches = data.filter(record => {
          ...
          if (businessLower && businessLower !== 'manufacturing') {
            ...
            if (marcusInsight.includes(businessLower) || 
              ...
              businessTypeRelevance = 1.5 // Boost relevant matches
            }
            ...
            if (hasConflictingContext) {
              businessTypeRelevance = 0.3 // Lower priority, don't exclude
            }
          }
  ...
  // STRATEGY 1: Database Intelligence - FULLY DYNAMIC SEARCH
  async classifyUsingDatabase(productDescription, businessType) {
    try {
      console.log('📊 Database Strategy: Dynamic search in 597K+ REAL trade_flows records')
      
      // Extract key terms for smart matching
      const keywords = this.extractSearchTerms(productDescription)
      const primaryKeywords = keywords.slice(0, 3) // Use top 3 keywords
      
      console.log(`🔍 Dynamic search for keywords: ${primaryKeywords.join(', ')}`)
      
      const suggestions = []
      
      // DYNAMIC SEARCH: Search enhanced database with timeout protection
      for (const keyword of primaryKeywords) {
        if (suggestions.length >= 5) break; // Limit total results
        
        // Add timeout wrapper to prevent database hangs
        const queryTimeout = 3000; // 3 second timeout
        
        console.log(`🔍 Searching for keyword: ${keyword}`)
        try {
          // Optimized query with fallback to ILIKE if textSearch fails
          let matches, error;
          
          // Try textSearch first (faster)
          const textSearchPromise = this.supabase
            .from('trade_flows')
            .select('hs_code, product_description')
            .textSearch('product_description', keyword, { type: 'websearch' })
            .not('product_description', 'is', null)
            .limit(8);
          ...
          try {
            ({ data: matches, error } = await Promise.race([textSearchPromise, timeoutPromise]));
          } catch (timeoutError) {
            // Fallback to simpler exact match query
            console.log(`⚠️ Keyword search timed out for "${keyword}", using fallback...`);
            ({ data: matches, error } = await this.supabase
          ...
          console.warn(`⚠️ Database query failed for "${keyword}":`, dbError.message);
          matches = null;
          error = dbError;
        }
        ...
        if (!error && matches?.length > 0) {
          ...
          // Process matches with improved deduplication
          const processedCodes = new Set();
          matches.forEach(match => {
            // Calculate dynamic confidence based on relevance
            let confidence = this.calculateDynamicConfidence(
              productDescription, 
            ...
            // Enhanced deduplication using Set for performance
            const formattedCode = this.formatHSCode(match.hs_code)
            if (!processedCodes.has(formattedCode) && confidence >= 70) {
              processedCodes.add(formattedCode)
              suggestions.push({
                code: this.formatHSCode(match.hs_code),
                description: match.product_description,
                confidence: Math.min(98, confidence),
                source: 'dynamic_database_search',
                matchedKeyword: keyword,
                strategy: 'fully_dynamic'
              })
            }
          })
        }
      ...
      console.log(`🎯 Dynamic search found ${sortedSuggestions.length} total matches`)
      
      // If 0 matches from database search, use business type intelligence
      if (sortedSuggestions.length === 0 && businessType) {
        console.log('🧠 Database search failed, applying business type intelligence...')
        const businessTypeResults = await this.generateBusinessTypeFallback(productDescription, businessType)
