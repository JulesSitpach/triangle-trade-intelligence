...
export const SimpleAuthProvider = ({ children }) => {
  ...
  const signUp = async (email, password, companyName, fullName) => {
    try {
      ...
      // ✅ CONVERSION FIX: If instant access granted (trial users), update user state immediately
      if (result.instant_access && result.user) {
        setUser(result.user);
        setSubscriptionTier(result.user.subscription_tier || 'Trial');
        console.log('✅ Trial registration + auto-login successful:', email, '| Instant access granted');
      } else {
        // For paid plans, just update tier (user still needs to verify email)
        setSubscriptionTier(result.profile?.subscription_tier || 'Trial');
        console.log('✅ Registration successful:', email, '| Email verification required');
      }
