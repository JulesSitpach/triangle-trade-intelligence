 ...
 *
 * @param {number} baseMfn - Base MFN rate (decimal)
 * @param {number} section301 - Section 301 rate (decimal)
 * @param {number} section232 - Section 232 rate (decimal)
 * @returns {number} Total rate (decimal)
 */
export function calculateTotalRate(baseMfn = 0, section301 = 0, section232 = 0) {
  const normalized = {
    baseMfn: normalizeRateToDecimal(baseMfn),
    section301: normalizeRateToDecimal(section301),
    section232: normalizeRateToDecimal(section232)
  };

  const total = normalized.baseMfn + normalized.section301 + normalized.section232;
