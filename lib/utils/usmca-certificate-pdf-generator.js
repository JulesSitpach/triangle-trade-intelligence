...
export async function generateUSMCACertificatePDF(certificateData, options = {}) {
  ...
  // Merge template fields with certificate data
  certificateData = {
    ...certificateData,
    ...templateFields,
    // Preserve original data that might override template defaults
    certifier: templateFields.certifier || certificateData.certifier,
    exporter: templateFields.exporter || certificateData.exporter,
    producer: templateFields.producer || certificateData.producer,
    importer: templateFields.importer || certificateData.importer
  };
  ...
  doc.setFontSize(7);
  // NO FALLBACKS - certifier country should always be provided
  ...
  if (!certificateData.certifier?.country) {
    const issueData = {
      ...
      has_certifier: !!certificateData.certifier,
      certifier_data: certificateData.certifier
    };
    console.error('🚨 DEV ISSUE: Missing certifier.country in certificate data', issueData);
    ...
    fetch('/api/admin/log-dev-issue', {
      ...
      body: JSON.stringify({
        ...
        message: 'Missing certifier.country in certificate data',
        data: issueData,
        certificate_number: certificateData.certificate_number
      })
  ...
  if (!certificateData.exporter?.country) {
    const issueData = {
      certificate_number: certificateData.certificate_number,
      has_exporter: !!certificateData.exporter,
      exporter_data: certificateData.exporter
    };
    console.error('🚨 DEV ISSUE: Missing exporter.country in certificate data', issueData);
    ...
    fetch('/api/admin/log-dev-issue', {
      ...
      body: JSON.stringify({
        ...
        severity: 'critical',
        component: 'pdf_generator',
        message: 'Missing exporter.country in certificate data',
        data: issueData,
        certificate_number: certificateData.certificate_number
      })
    }).catch(err => console.error('Failed to log dev issue:', err));
  ...
  doc.text('COUNTRY', margin + 1, y + 26);
  doc.setFontSize(7);
  // NO FALLBACKS - if producer is same as exporter, use exporter country, otherwise require producer country
  const prodCountry = certificateData.producer?.same_as_exporter
    ? (certificateData.exporter?.country || '⚠️ ERROR: Missing exporter country')
    : (certificateData.producer?.country || '⚠️ ERROR: Missing producer country');
  if (!certificateData.producer?.country && !certificateData.producer?.same_as_exporter) {
    const issueData = {
      certificate_number: certificateData.certificate_number,
      producer_same_as_exporter: certificateData.producer?.same_as_exporter,
      has_producer: !!certificateData.producer,
      producer_data: certificateData.producer
    };
    console.error('🚨 DEV ISSUE: Missing producer.country in certificate data', issueData);
    ...
    fetch('/api/admin/log-dev-issue', {
      ...
      body: JSON.stringify({
        ...
        severity: 'critical',
        component: 'pdf_generator',
        message: 'Missing producer.country in certificate data',
        data: issueData,
        certificate_number: certificateData.certificate_number
      })
    }).catch(err => console.error('Failed to log dev issue:', err));
