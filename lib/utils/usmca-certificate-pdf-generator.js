...
export async function generateUSMCACertificatePDF(certificateData, options = {}) {
  ...
  try {
    ...
    // ✅ REMOVED: Horizontal line that was blocking 12e/f/g fields
    // The line created visual clutter and blocked the date, telephone, email fields
    authY += 2; // Minimal space before fields (reduced from 6)

    // 12e, 12f, 12g (center-aligned thirds layout for professional appearance)
    const thirdWidth = contentWidth / 3;
    doc.setFontSize(8);
    doc.setFont(undefined, 'bold');

    // Center-align labels in each third
    doc.text('12e. DATE (MM/DD/YYYY)', LAYOUT.page.margin + (thirdWidth / 2), authY, { align: 'center' });
    doc.text('12f. TELEPHONE NUMBER', LAYOUT.page.margin + thirdWidth + (thirdWidth / 2), authY, { align: 'center' });
    doc.text('12g. EMAIL', LAYOUT.page.margin + (thirdWidth * 2) + (thirdWidth / 2), authY, { align: 'center' });

    authY += 5; // More space before values
    doc.setFont(undefined, 'normal');
    doc.setFontSize(9);

    // Center-align values in each third
    doc.text(certificateData.authorization?.signature_date || certificateData.signature_date || new Date().toLocaleDateString('en-US'),
      LAYOUT.page.margin + (thirdWidth / 2), authY, { align: 'center' });
    doc.text(certificateData.authorization?.phone || certificateData.signatory_phone || '',
      LAYOUT.page.margin + thirdWidth + (thirdWidth / 2), authY, { align: 'center' });
    doc.text(certificateData.authorization?.email || certificateData.signatory_email || '',
      LAYOUT.page.margin + (thirdWidth * 2) + (thirdWidth / 2), authY, { align: 'center' });
