...
export async function generateUSMCACertificatePDF(certificateData, options = {}) {
  ...
  try {
    ...
    const renderPartySection = (title, data, x, yStart, width, height, drawBorders) => {
      ...
      // Light gray background for section title (professional look - increased height for breathing room)
      doc.setFillColor(...LAYOUT.colors.sectionHeaders);
      doc.rect(x, yStart, width, 4.5, 'F');

      doc.setFontSize(8);
      doc.setFont(undefined, 'bold');
      doc.text(title, x + 3, fieldY);
      fieldY += 5;

    doc.setFontSize(8);
    doc.setFont(undefined, 'normal');
    doc.text('NAME', x + 3, fieldY);
    fieldY += 3.5;
    doc.setFontSize(9);
    doc.text(data?.name || '', x + 3, fieldY);
    fieldY += 5;

    doc.setFontSize(8);
    doc.text('ADDRESS', x + 3, fieldY);
    fieldY += 3.5;
    doc.setFontSize(9);
    const addr = wrapText(data?.address || '', width - 6);
    doc.text(addr, x + 3, fieldY);
    fieldY += 9;

    doc.setFontSize(8);
    doc.text('COUNTRY', x + 3, fieldY);
    doc.text('PHONE', x + width/2, fieldY);
    fieldY += 3.5;
    doc.setFontSize(9);
    doc.text(data?.country || '', x + 3, fieldY);
    doc.text(data?.phone || '', x + width/2, fieldY);
    fieldY += 5;

    doc.setFontSize(8);
    doc.text('EMAIL', x + 3, fieldY);
    fieldY += 3.5;
    doc.setFontSize(9);
    doc.text(data?.email || '', x + 3, fieldY);
    fieldY += 5;

    doc.setFontSize(8);
    doc.text('TAX IDENTIFICATION NUMBER', x + 3, fieldY);
    fieldY += 3.5;
    doc.setFontSize(9);
    doc.text(data?.tax_id || '', x + 3, fieldY);
  };
    ...
    doc.text('7.', col7X + 3, tableTop + 4);
    doc.text('HTS', col7X + 3, tableTop + 7);
    doc.text('8. ORIGIN', col8X + 3, tableTop + 4);
    doc.text('CRITERION', col8X + 3, tableTop + 7);
    doc.text('9. PRODUCER', col9X + 2, tableTop + 4);
    doc.text('(YES/NO)', col9X + 2, tableTop + 7);
    doc.text('10. METHOD OF', col10X + 3, tableTop + 4);
    doc.text('QUALIFICATION', col10X + 3, tableTop + 7);
    doc.text('11. COUNTRY', col11X + 3, tableTop + 4);
    doc.text('OF ORIGIN', col11X + 3, tableTop + 7);

    // Data row
    doc.setFont(undefined, 'normal');
    doc.setFontSize(9);
    const dataY = tableTop + LAYOUT.sections.productTable.headerHeight + 5;
    ...
    doc.text(hsCode, col7X + 3, dataY);

    // Field 8: Origin Criterion
    doc.text(certificateData.preference_criterion || certificateData.origin_criterion || '', col8X + 7, dataY);

    // Field 9: Producer
    const isProducer = certificateData.producer_declaration?.is_producer || certificateData.is_producer;
    doc.text(isProducer ? 'YES' : 'NO', col9X + 5, dataY);

    // Field 10: Method
    doc.text(certificateData.qualification_method?.method || certificateData.qualification_method || 'RVC', col10X + 3, dataY);

    // Field 11: Country
    doc.text(certificateData.country_of_origin || '', col11X + 3, dataY);
    ...
    doc.setFontSize(9);
    // Signature box (left)
    doc.rect(LAYOUT.page.margin + 3, authY, halfWidth - 6, 10);
    // Company name (right)
    doc.text(certificateData.exporter?.name || '', LAYOUT.page.margin + halfWidth + 3, authY + 5);
    authY += 13;
    ...
    doc.setFontSize(8);
    doc.setFont(undefined, 'bold');
    doc.text('12g. EMAIL', LAYOUT.page.margin + 3, authY);
    authY += 4;
    doc.setFont(undefined, 'normal');
    doc.setFontSize(9);
    doc.text(certificateData.authorization?.email || certificateData.signatory_email || '', LAYOUT.page.margin + 3, authY);
