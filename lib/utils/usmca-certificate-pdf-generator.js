...
export async function generateUSMCACertificatePDF(certificateData, options = {}) {
  ...
  // ⚡ IMPROVED: Better table layout with proper cell spacing (matching EditableCertificatePreview styling)
  const tableHeight = 60;

  // Main table border (outer rectangle)
  doc.rect(margin, y, contentWidth, tableHeight);

  // Table column positions (adjusted for better visual spacing)
  const col6X = margin + 1;
  const col7X = margin + 67;
  const col8X = margin + 87;
  const col9X = margin + 107;
  const col10X = margin + 137;
  const col11X = margin + 172;
  ...
  // Table headers (Field labels with number and description)
  doc.setFontSize(6);
  doc.setFont(undefined, 'bold');

  // Field 6: Description of Goods
  doc.text('6.', col6X + 1, y + 2);
  doc.text('DESCRIPTION', col6X + 1, y + 5);
  doc.text('OF GOOD(S)', col6X + 1, y + 8);

  // Field 7: HTS/HS Code
  doc.text('7.', col7X + 1, y + 2);
  doc.text('HTS', col7X + 1, y + 5);
  doc.text('CODE', col7X + 1, y + 8);

  // Field 8: Origin Criterion - CLEARLY VISIBLE
  doc.text('8.', col8X + 2, y + 2);
  doc.text('ORIGIN', col8X + 1, y + 5);
  doc.text('CRITERION', col8X + 0.5, y + 8);

  // Field 9: Producer Declaration
  doc.text('9.', col9X + 1, y + 2);
  doc.text('PRODUCER', col9X + 0.5, y + 5);
  doc.text('(YES/NO)', col9X + 0.5, y + 8);

  // Field 10: Method of Qualification
  doc.text('10.', col10X, y + 2);
  doc.text('METHOD OF', col10X + 0.5, y + 5);
  doc.text('QUALIFICATION', col10X - 1, y + 8);

  // Field 11: Country of Origin
  doc.text('11.', col11X + 1, y + 2);
  doc.text('COUNTRY', col11X + 0.5, y + 5);
  doc.text('OF ORIGIN', col11X - 0.5, y + 8);
  ...
  // DEBUG: Log field values to verify data is flowing through PDF generator
  console.log('📄 PDF Generator - Field Values:', {
    'FIELD 6 (Product Description)': certificateData.product?.description || certificateData.product_description || '',
    'FIELD 7 (HS Code)': certificateData.hs_classification?.code || certificateData.product?.hs_code || '',
    'FIELD 8 (Origin Criterion) - preference_criterion': certificateData.preference_criterion,
    'FIELD 8 (Origin Criterion) - origin_criterion': certificateData.origin_criterion,
    'FIELD 9 (Producer)': certificateData.producer_declaration?.is_producer,
    'FIELD 10 (Method)': certificateData.qualification_method?.method,
    'FIELD 11 (Country)': certificateData.country_of_origin,
    'Full certificateData keys': Object.keys(certificateData)
  });

  // ========== FIELD 6: Product Description ==========
  // Keep single line in PDF (no wrapping, consistent with preview)
  const productDesc = (certificateData.product?.description || certificateData.product_description || '').substring(0, 60);
  doc.text(productDesc, col6X + 1, dataY);
  ...
  // ========== FIELD 8: Origin Criterion (A, B, C, or D) ==========
  // ✅ CRITICAL: This is what user was looking for
  // Priority: preference_criterion (from professional_certificate) → origin_criterion (from EditableCertificatePreview edits)
  const originCriterion = certificateData.preference_criterion || certificateData.origin_criterion || '';
  console.log('📄 ✅ FIELD 8 ORIGIN CRITERION RENDERING:', {
    originCriterion,
    column_position: col8X,
    y_position: dataY,
    value_present: !!originCriterion
  });
  ...
  // ========== FIELD 9: Producer Declaration (YES/NO) ==========
  const producerText = certificateData.producer_declaration?.is_producer ? 'YES' : 'NO';
  const col9Width = col10X - col9X;
  const centerX9 = col9X + (col9Width / 2) - 1;
  doc.text(producerText, centerX9, dataY, { align: 'center' });

  // ========== FIELD 10: Method of Qualification ==========
  const method = certificateData.qualification_method?.method || 'RVC';
  doc.text(method, col10X + 1, dataY);

  // ========== FIELD 11: Country of Origin ==========
  const countryOfOrigin = certificateData.country_of_origin || '';
  doc.text(countryOfOrigin, col11X + 2, dataY);
