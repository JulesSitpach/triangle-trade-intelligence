 ...
 *
 * Optimized layout with compressed middle sections and spacious authorization.
 *
 * Layout Optimizations:
 * - Header: 16mm (compressed from 18mm) - tighter spacing
 * - Section 1 (Certifier Type): 18mm (compressed from 20mm)
 * - Sections 2-5 (Party Info): 44mm each (compressed from 48mm)
 * - Product Table: 38mm (compressed from 50mm) - more compact
 * - Certification Statement: 14mm (compressed from 16mm) - 7pt font (very compact)
 * - Authorization: 60mm (kept spacious) - professional appearance
 *
 * Professional Features:
 * - Centered table columns for clean scannable layout
 * - Centered authorization bottom row (12e/f/g) for balanced appearance
 ...
 * - Compact field spacing: 2.5mm label gap, 4mm value gap
 * - Border widths: 1mm main, 0.7mm sections, 0.3mm internal
 * - Font sizes: 13px header, 12px subheader, 9px data, 8px labels, 7px legal text
 *
 * October 30, 2025 - Final polish: centered auth fields, minimal legal text
 */
...
export async function generateUSMCACertificatePDF(certificateData, options = {}) {
  ...
  try {
    ...
    const LAYOUT = {
      ...
      sections: {
        header: { height: 16 },        // Reduced from 18 (compress header)
        certifierType: { height: 18, width: 70 }, // Reduced from 20 (compress Section 1)
        partyInfo: { height: 44 },     // Reduced from 48 (compress party sections)
        productTable: { height: 38, headerHeight: 8 },  // Reduced from 50 (compress table)
        certStatement: { height: 14 }, // Reduced from 16 (compress statement)
        authorization: { height: 60 }  // Keep at 60 for proper spacing
      },
      borders: {
    ...
    // Left side: Certifier Type (no border - already inside main border)
    doc.setFontSize(8);
    doc.setFont(undefined, 'bold');
    ...
    // Auto-generate one-year blanket period if not provided
    ...
    const blanketFrom = certificateData.blanket_period?.start_date ||
                       certificateData.blanket_from ||
                       formatDate(today);
    const blanketTo = certificateData.blanket_period?.end_date ||
                     certificateData.blanket_to ||
                     formatDate(oneYearLater);

    doc.setFillColor(...LAYOUT.colors.blanketPeriod);
    doc.rect(bpX, section1Top, bpWidth, LAYOUT.sections.certifierType.height, 'F'); // F = Fill only (no border - main border covers it)

    // Vertical line separating Section 1 from Blanket Period
    doc.setLineWidth(LAYOUT.borders.internal);
    doc.setDrawColor(0, 0, 0);
    doc.line(bpX, section1Top, bpX, section1Top + LAYOUT.sections.certifierType.height);
    ...
    doc.setFont(undefined, 'normal');
    doc.text(blanketFrom, bpX + 15, section1Top + 12);

    doc.setFont(undefined, 'bold');
    doc.text('TO:', bpX + 3, section1Top + 16);
    doc.setFont(undefined, 'normal');
    doc.text(blanketTo, bpX + 15, section1Top + 16);
    ...
    const renderPartySection = (title, data, x, yStart, width, height, useAlternateBackground = false) => {
      ...
      // CONSISTENT SPACING CONSTANTS (COMPACT)
      const FIELD_SPACING = {
        headerHeight: 5,      // Section header background height (reduced)
        labelGap: 2.5,        // Gap between label and value (reduced)
        valueGap: 4,          // Gap after value before next label (reduced)
    ...
    // Draw the 2x2 grid dividing lines (thin)
    doc.setLineWidth(LAYOUT.borders.internal);
    doc.setDrawColor(0, 0, 0);
    const gridStartY = y - LAYOUT.sections.partyInfo.height * 2;
    // Horizontal line between rows
    doc.line(LAYOUT.page.margin, y - LAYOUT.sections.partyInfo.height, LAYOUT.page.width - LAYOUT.page.margin, y - LAYOUT.sections.partyInfo.height);
    // Vertical line down the middle
    doc.line(LAYOUT.page.margin + halfWidth, gridStartY, LAYOUT.page.margin + halfWidth, y);
    ...
    // Data row (centered in columns) - compact spacing
    doc.setFont(undefined, 'normal');
    doc.setFontSize(9);
    ...
    // ========== CERTIFICATION STATEMENT (VERY COMPACT) ==========
    doc.setFontSize(7);  // Smaller font for maximum space savings
    doc.setFont(undefined, 'normal');
    ...
    doc.text(wrappedCert, LAYOUT.page.margin + 2, y + 4);  // Less padding
    ...
    // ========== BOTTOM CONTACT INFO BOX (12e, 12f, 12g) ==========
    y += 4; // Space before contact box
    const contactBoxTop = y;
    const contactBoxHeight = 16;

    // Contact box background (light gray)
    doc.setFillColor(248, 248, 248);
    doc.rect(LAYOUT.page.margin, contactBoxTop, contentWidth, contactBoxHeight, 'F');

    // Contact box border
    doc.setLineWidth(LAYOUT.borders.section);
    doc.setDrawColor(0, 0, 0);
    doc.rect(LAYOUT.page.margin, contactBoxTop, contentWidth, contactBoxHeight);

    // Divide into thirds with vertical lines
    const thirdWidth = contentWidth / 3;
    doc.setLineWidth(LAYOUT.borders.internal);
    doc.line(LAYOUT.page.margin + thirdWidth, contactBoxTop, LAYOUT.page.margin + thirdWidth, contactBoxTop + contactBoxHeight);
    doc.line(LAYOUT.page.margin + (thirdWidth * 2), contactBoxTop, LAYOUT.page.margin + (thirdWidth * 2), contactBoxTop + contactBoxHeight);

    let contactY = contactBoxTop + 5;

    // Labels (12e, 12f, 12g)
    doc.setFontSize(8);
    doc.setFont(undefined, 'bold');
    doc.text('12e. DATE (MM/DD/YYYY)', LAYOUT.page.margin + (thirdWidth / 2), contactY, { align: 'center' });
    doc.text('12f. TELEPHONE NUMBER', LAYOUT.page.margin + thirdWidth + (thirdWidth / 2), contactY, { align: 'center' });
    doc.text('12g. EMAIL', LAYOUT.page.margin + (thirdWidth * 2) + (thirdWidth / 2), contactY, { align: 'center' });
    ...
    doc.setFont(undefined, 'normal');
    doc.setFontSize(9);
    doc.text(certificateData.authorization?.signature_date || certificateData.signature_date || new Date().toLocaleDateString('en-US'),
      LAYOUT.page.margin + (thirdWidth / 2), contactY, { align: 'center' });
    doc.text(certificateData.authorization?.phone || certificateData.signatory_phone || '',
      LAYOUT.page.margin + thirdWidth + (thirdWidth / 2), contactY, { align: 'center' });
    doc.text(certificateData.authorization?.email || certificateData.signatory_email || '',
      LAYOUT.page.margin + (thirdWidth * 2) + (thirdWidth / 2), contactY, { align: 'center' });

    y = contactBoxTop + contactBoxHeight;

    // ========== FOOTER ==========
    // (Footer removed - clean professional certificate)
    const footerY = LAYOUT.page.height - LAYOUT.page.margin - 3;
