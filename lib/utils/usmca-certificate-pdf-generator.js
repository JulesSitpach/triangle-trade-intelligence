...
export async function generateUSMCACertificatePDF(certificateData, options = {}) {
  ...
  // Column 6 - Description (Product Description)
  const description = wrapText(certificateData.product?.description || certificateData.product_description || '', 64);
  ...
  doc.text(hsCode, col7X + 1, dataY);

  // ✅ Column 8 - Origin Criterion (FIELD 8)
  ...
  console.log('📄 FIELD 8 Rendering:', { originCriterion, col8X, dataY });
  doc.text(originCriterion, col8X + 2, dataY); // Slight offset for better centering in column

  // Column 9 - Producer (YES/NO)
  const producerText = certificateData.producer_declaration?.is_producer ? 'YES' : 'NO';
  doc.text(producerText, col9X + 1, dataY);

  // Column 10 - Method of Qualification
  const method = wrapText(certificateData.qualification_method?.method || 'RVC', 32);
  doc.text(method, col10X + 1, dataY);

  // Column 11 - Country of Origin
  const countryOfOrigin = certificateData.country_of_origin || '';
  doc.text(countryOfOrigin, col11X + 1, dataY);
