...
export async function generateUSMCACertificatePDF(certificateData, options = {}) {
  ...
  try {
    ...
    // This certifies that the product qualifies as originating (meets RVC threshold)
    const isQualified = certificateData.qualification_status === 'QUALIFIED' ||
                        certificateData.qualified === true ||
                        certificateData.usmca?.qualified === true;
    const isProducer = isQualified ? 'YES' : 'NO';
