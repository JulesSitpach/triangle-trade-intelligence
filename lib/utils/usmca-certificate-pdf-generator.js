...
export async function generateUSMCACertificatePDF(certificateData, options = {}) {
  ...
  try {
    ...
    doc.setFontSize(8);
    const certNum = certificateData.certificate_number || `USMCA-${new Date().getFullYear()}-${Math.floor(Math.random() * 100000)}`;
    ...
    const renderParty = (sectionNum, title, data, x, yStart) => {
      ...
      doc.setFontSize(8);
      doc.setFont(undefined, 'bold');
      doc.text(data?.country || '', x + padding, fieldY);
    ...
    if (watermark && userTier === 'Trial') {
      ...
      doc.setFont(undefined, 'bold');
      doc.text('TRIAL VERSION', PAGE.width / 2, PAGE.height / 2, {
        align: 'center',
        angle: 45
      });
