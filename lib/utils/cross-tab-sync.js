...
export class CrossTabSync {
    ...
    this.storageListener = (event) => {
      ...
      try {
        ...
        if (timeSinceLastUpdate < 5000) {
          ...
          // ✅ P1-4 FIX: Show user-visible warning for conflicts
          if (typeof window !== 'undefined' && window.confirm) {
            // Use confirm() instead of alert() so user can dismiss it
            const userAcknowledged = window.confirm(
              '⚠️ Multi-Tab Edit Detected\n\n' +
              'Another browser tab just modified this workflow.\n\n' +
              'Your recent changes may have been overwritten.\n\n' +
              'RECOMMENDATION: Close other tabs editing this workflow to prevent data loss.\n\n' +
              'Click OK to continue (newest changes will be kept).'
            );
            console.log(`[CrossTabSync] User acknowledged conflict: ${userAcknowledged}`);
          }
   ...
   */
  destroy() {
    if (this.enabled && this.storageListener) {
      // ✅ FIX: Remove event listener to prevent memory leak
      window.removeEventListener('storage', this.storageListener);
      this.storageListener = null;
      console.log(`[CrossTabSync] Destroyed for tab ${this.tabId} - listener removed`);
    }
  }
}
