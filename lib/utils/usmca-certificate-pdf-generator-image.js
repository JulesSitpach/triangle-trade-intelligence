...
export async function generateCertificatePDFFromImage(imageDataUrl, options = {}) {
  ...
  console.log('🎨 PDF Generation:', { watermark, userTier, options });
  ...
  try {
    ...
    // Add blurred watermark if trial user (case-insensitive check)
    const tierLower = (userTier || '').toLowerCase();
    const shouldAddWatermark = watermark && (tierLower === 'trial' || tierLower === 'free');

    console.log('💧 Watermark check:', { watermark, userTier, tierLower, shouldAddWatermark });
    ...
    if (shouldAddWatermark) {
      ...
      doc.setFontSize(52);
      doc.setTextColor(200, 200, 200, 0.3); // Light gray, 30% opacity
      doc.text('TRIAL VERSION', pageWidth / 2, pageHeight / 2 - 10, {
        align: 'center',
        angle: 45
      });
      ...
      doc.setFontSize(50);
      doc.setTextColor(210, 210, 210, 0.5); // Light gray, 50% opacity
      doc.text('TRIAL VERSION', pageWidth / 2, pageHeight / 2 - 10, {
        align: 'center',
        angle: 45
      });
      ...
      doc.setFontSize(48);
      doc.setTextColor(220, 220, 220, 0.8); // Light gray, 80% opacity
      doc.text('TRIAL VERSION', pageWidth / 2, pageHeight / 2 - 10, {
        align: 'center',
        angle: 45
      });
      ...
      doc.text('NOT VALID FOR CUSTOMS', pageWidth / 2, pageHeight / 2 + 15, {
        align: 'center',
        angle: 45
      });

      doc.setFontSize(28);
      doc.setTextColor(210, 210, 210, 0.5);
      doc.text('NOT VALID FOR CUSTOMS', pageWidth / 2, pageHeight / 2 + 15, {
        align: 'center',
        angle: 45
      });
      ...
      doc.setFontSize(26);
      doc.setTextColor(220, 220, 220, 0.8);
      doc.text('NOT VALID FOR CUSTOMS', pageWidth / 2, pageHeight / 2 + 15, {
        align: 'center',
        angle: 45
      });
