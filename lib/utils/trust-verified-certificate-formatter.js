...
${/* ✅ REMOVED: || 'B' default (line 78) - REASON: FALSE CERTIFICATION */ cert.preference_criterion || cert.additional_information?.preference_criterion || '⚠️ NOT DETERMINED BY AI'} - ${cert.preference_explanation || 'The good is originating because it satisfies the requirements for preferential tariff treatment under the USMCA'}
...
═══════════════════════════════════════════════════════════════════════════
Preference Criterion: ${/* ✅ REMOVED: || 'B' default (line 192) */ cert.preference_criterion || '⚠️ NOT DETERMINED BY AI'}
...
HS Code: ${cert.product?.hs_code || 'TO BE VERIFIED'}
Preference Criterion: ${/* ✅ REMOVED: || 'B' default (line 229) */ cert.preference_criterion || '⚠️ NOT DETERMINED BY AI'}
...
obtain proper signatures before submission to customs authorities.`;
  ...
  formatExpertValidationSection(expertValidation) {
    ...
    if (expertValidation.expert_contact_info) {
      ...
      lines.push('  - Expert Contact: Consult trade compliance expert');
    }
  ...
  formatCertificateReadinessAssessment(cert) {
    ...
    if (cert.trust_verification?.expert_validation?.expert_validation_required) {
      assessment.push('⚠ EXPERT REVIEW REQUIRED: Contact trade compliance expert');
    } else {
