 ...
 * Keeps fields in snake_case throughout (database format is canonical)
 *
 * @param {Object} apiComponent - API response component
 * @returns {Object} Frontend-ready component with snake_case fields
 *
 * @example
 *   const apiComponent = {
 *     mfn_rate: 25,
 ...
 *   // Result (same as input - snake_case is canonical):
 *   // {
 *   //   mfn_rate: 25,
 *   //   usmca_rate: 0,
 *   //   origin_country: 'CN',
...
export function transformAPIToFrontend(apiComponent) {
  ...
  Object.entries(COMPONENT_DATA_CONTRACT.fields).forEach(([dbFieldName, fieldDef]) => {
    ...
    // Handle missing fields - validate required fields exist
    ...
    try {
      // Transform to frontend value (type conversion only, no field renaming)
      ...
      // ✅ SIMPLIFIED: Use snake_case for all field names (database canonical format)
      // Frontend and backend both use same field names - eliminates confusion
      frontendComponent[dbFieldName] = frontendValue;

      // Validate the value
      const validation = COMPONENT_DATA_CONTRACT.validate(dbFieldName, frontendValue);
      if (!validation.valid) {
        errors.push(`${dbFieldName}: ${validation.errors.join('; ')}`);
      }
    } catch (error) {
