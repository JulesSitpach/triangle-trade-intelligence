...
export function transformAPIToFrontend(apiComponent) {
  ...
  Object.entries(COMPONENT_DATA_CONTRACT.fields).forEach(([dbFieldName, fieldDef]) => {
    let apiValue = apiComponent[dbFieldName];

    // Handle missing fields - use fallback for required fields
    if (apiValue === undefined) {
      if (fieldDef.required) {
        if (fieldDef.fallback !== undefined) {
          // ✅ FIX: Use the fallback value from contract instead of skipping
          apiValue = fieldDef.fallback;
        } else {
          errors.push(`Missing required field: ${dbFieldName}`);
          return;
        }
    ...
    try {
      ...
      // ✅ CRITICAL FIX (Oct 26): Add BOTH database field name and frontend field name
      // This ensures:
      // 1. Frontend receives camelCase names (mfn_rate → mfnRate)
      // 2. Backend/database code can still find snake_case names (hs_code, origin_country)
      // Without this, components are received by workflow-session without hs_code/origin_country

      // Add database field name (for backend code like workflow-session.js)
      frontendComponent[dbFieldName] = frontendValue;

      // Add frontend field name (for React components)
