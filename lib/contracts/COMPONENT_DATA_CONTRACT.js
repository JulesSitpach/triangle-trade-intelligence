...
export const COMPONENT_DATA_CONTRACT = {
  ...
  fields: {
    ...
    mfn_rate: {
      ...
      types: {
        database: 'NUMERIC(5,2)',     // 999.99 max
        api: 'number',                 // Send as number, not string
        frontend: 'number',
        ai: 'number',
      },
      ...
      transform: {
        database_to_api: (val) => {
          // Database returns string "25.00", convert to number
    ...
    savings_percentage: {
      ...
      example: 100,  // 100% savings if USMCA qualified
      fallback: 0,
    },
    ...
    hs_code: {
      ...
      transform: {
        database_to_api: (val) => val?.replace(/\D/g, '') || '',
        api_to_frontend: (val) => val || '',
        ai_to_database: (val) => val?.replace(/\D/g, '').substring(0, 10) || '',
      },
    ...
    base_mfn_rate: {
      ...
      note: 'Should be same as mfn_rate in most cases, separated for clarity on savings',
      example: 25,
      fallback: 0,
    },
