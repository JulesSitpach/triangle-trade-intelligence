...
export const COMPONENT_DATA_CONTRACT = {
  ...
  fields: {
    ...
    mfn_rate: {
      ...
      transform: {
        database_to_api: (val) => {
          // ✅ FIX (Oct 28): Handle BOTH percentage (25.00) AND decimal (0.60) formats from database
          // Database stores decimals: "0.60" → API: 0.60 (already decimal, don't divide)
          // Database stores percentages: "25.00" → API: 0.25 (divide by 100)
          const num = parseFloat(val);
          if (isNaN(num)) throw new Error(`Invalid MFN rate: ${val}`);

          // If already in decimal range (0-1), return as-is
          // If in percentage range (1-100), divide by 100
          if (num >= 0 && num <= 1) {
            return num;  // Already decimal format
          } else if (num > 1) {
    ...
    usmca_rate: {
      ...
      transform: {
        database_to_api: (val) => {
          // ✅ FIX (Oct 28): Handle BOTH percentage (25.00) AND decimal (0.00) formats from database
          const num = parseFloat(val);
    ...
    total_rate: {
      ...
      names: {
        database: 'total_rate',
        api: 'total_rate',
        frontend: 'total_rate',  // ✅ FIXED: Use snake_case (not camelCase) per CLAUDE.md rules
    ...
    // ==================== AI VALIDATION FIELDS (Oct 28, 2025) ====================
    // NEW: Added to enforce AI validation of zero rates instead of passive acceptance
    // These fields provide audit trail explaining why each rate is correct

    justification: {
      label: 'Rate Justification',
      category: 'metadata',
      description: 'AI explanation of why this tariff rate is correct (especially for zero rates)',
      ...
      transform: {
        database_to_api: (val) => val || '',
        api_to_frontend: (val) => val || '',
        ai_to_database: (val) => val?.substring(0, 1000) || '',
      },

      example: 'Confirmed ITA duty-free: Information Technology Agreement covers semiconductor processors',
      fallback: 'No justification provided',
      note: 'CRITICAL: If MFN rate is 0%, AI must explicitly state WHY (ITA exemption, duty-free item, etc.)'
    },

    confidence: {
      label: 'AI Confidence in Rate',
      category: 'metadata',
      description: 'AI confidence level in this tariff rate (high/medium/low)',
      ...
      transform: {
        database_to_api: (val) => val || 'low',
        api_to_frontend: (val) => val || 'low',
        ai_to_database: (val) => {
          const valid = ['high', 'medium', 'low'];
          return valid.includes(val?.toLowerCase()) ? val.toLowerCase() : 'low';
        },
      },

      example: 'high',
      fallback: 'low',
      note: 'High=clear research confirms rate, Medium=reasonable confidence with minor uncertainties, Low=multiple interpretations or incomplete data'
    },

    data_quality_flag: {
      label: 'Data Quality Issues',
      category: 'metadata',
      description: 'Flag indicating any data quality issues or uncertainties found',

      names: {
        database: 'data_quality_flag',
        api: 'data_quality_flag',
        frontend: 'data_quality_flag',
        ai: 'data_quality_flag',
      },
      ...
      transform: {
        database_to_api: (val) => val || null,
        api_to_frontend: (val) => val || null,
        ai_to_database: (val) => val?.substring(0, 500) || null,
      },

      example: 'Database shows 0% but actual USITC rate is 5% - recommend verification',
      fallback: null,
      note: 'Only present if issues found. Null = no issues detected. Alerts: incomplete data, conflicting rates, or uncertain classification'
    },
  },
