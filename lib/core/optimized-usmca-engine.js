...
export class OptimizedUSMCAEngine {
  ...
  /**
   * Get USMCA threshold using centralized database-driven system
   */
  async getUSMCAThresholdForBusinessType(businessType) {
    try {
      // Use centralized threshold lookup from config/usmca-thresholds.js
      const { getUSMCAThreshold } = await import('../../config/usmca-thresholds.js');
      const thresholdData = await getUSMCAThreshold(businessType);
      
      console.log(`📋 USMCA Threshold: ${businessType} → ${thresholdData.threshold}% (${thresholdData.source})`);
      
      return {
        threshold: thresholdData.threshold,
        source: thresholdData.source || 'database_driven_lookup',
        rule_type: thresholdData.rule_type || 'regional_value_content',
        documentation: thresholdData.documentation
      };
      
    } catch (error) {
      console.error('Database threshold lookup failed, using fallback:', error);
      
      // Emergency fallback with correct thresholds
      const emergencyThresholds = {
        'Electronics': 65.0,
        'Electronics & Technology': 65.0,
        'Automotive': 75.0,
        'Textiles': 55.0,
        'Textiles & Apparel': 55.0,
        'Manufacturing': 62.5
      };
      
      const fallbackThreshold = emergencyThresholds[businessType] || 62.5;
      
      return {
        threshold: fallbackThreshold,
        source: 'emergency_fallback_correct_values',
