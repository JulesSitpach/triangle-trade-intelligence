...
export class DatabaseDrivenUSMCAEngine {
  ...
  /**
   * Get USMCA qualification rules from config file (HYBRID APPROACH)
   * Database used for user data, config file for industry thresholds
   */
  async getQualificationRules(hsCode = null, businessType = null) {
    ...
    try {
      // HYBRID APPROACH: Use config file for thresholds (fixed by industry)
      // Import dynamically to avoid top-level async issues
      const { getUSMCAThreshold } = await import('../../config/usmca-thresholds.js');

      // Get threshold from config file based on business type
      const thresholdData = await getUSMCAThreshold(businessType, hsCode);

      // Convert config data to rule format expected by engine
      const rule = {
        rule_type: thresholdData.rule_type || 'regional_value_content',
        regional_content_threshold: thresholdData.threshold,
        required_documentation: thresholdData.documentation || [
        ...
        hs_code: hsCode,
        is_default: !businessType // Only default if no business type provided
      };

      logInfo('Using config file threshold for USMCA rules', {
        businessType,
        hsCode,
        threshold: rule.regional_content_threshold,
        source: rule.source
      });

      this.setCache(cacheKey, rule);
      return rule;

    } catch (error) {
      logError('Failed to retrieve USMCA qualification rules from config', {
        error: error.message,
        hsCode,
        businessType
      });
      throw new Error(`USMCA rules lookup failed: ${error.message}`);
    }
  }
  ...
  async getTariffRates(hsCode, destinationCountry = SAFE_COUNTRIES.defaultDestination) {
    ...
    try {
      ...
      logError('No tariff rates found in database - system incomplete', { hsCode, destinationCountry });
  ...
  calculateRegionalContent(componentOrigins, usmcaCountries, rules, manufacturingLocation) {
    console.log('🧮 ========== CALCULATING REGIONAL CONTENT ==========');
    ...
    console.log('📦 Component Origins received:', componentOrigins.map((c, i) => ({
      index: i + 1,
      origin_country: c.origin_country,
      value_percentage: c.value_percentage,
      is_usmca: usmcaCountries.includes(c.origin_country)
    })));
    ...
    console.log('➕ Total value percentage:', totalValue);
    ...
    console.log('🇺🇸🇲🇽🇨🇦 North American value:', northAmericanValue);
    console.log('📊 North American percentage:', northAmericanPercentage.toFixed(2) + '%');

    // Apply threshold from database rules
    const threshold = rules.regional_content_threshold;
    console.log('🎯 Threshold from config file:', threshold + '%');
    console.log('✓ Qualified?', northAmericanPercentage >= threshold);
