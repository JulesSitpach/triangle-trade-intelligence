...
export class DynamicSavingsEngine extends DynamicEngineBase {
   ...
   * Calculates savings for multiple routes in a single operation
   * Replaces individual calculateRouteSavings calls with optimized batch processing
   */
  async calculateBatchRouteSavings(batchContext) {
    const startTime = Date.now()
    const routes = batchContext.routes || []
    ...
    try {
      ...
      // Extract common parameters for batch processing
      const { importVolume, businessProfile, products } = batchContext
      const parsedVolume = this.parseImportVolume(importVolume)
      
      // Get base business profile calculations once for all routes
      const baseCalculations = {
        processSavings: this.calculateProcessOptimizationSavings(businessProfile),
        timeValueSavings: this.calculateTimeValueSavings(businessProfile),
        riskMitigationValue: this.calculateRiskMitigationValue(businessProfile),
        marketMultiplier: this.getCurrentMarketMultiplier(businessProfile.businessType)
      }

      // Batch process all routes with shared calculations
      const batchResults = routes.map((route, index) => {
        // Calculate route-specific tariff savings
        const routeSpecificSavings = this.calculateTariffSavings(
          businessProfile, 
          route.type || 'triangle'
        )

        // Apply route-specific multipliers
        const routeMultiplier = this.getRouteTypeMultiplier(route.type, route.usmcaTariff)
        
        // Calculate total savings for this specific route
        const totalSavings = Math.round(
          (routeSpecificSavings + 
           baseCalculations.processSavings + 
           baseCalculations.timeValueSavings + 
           baseCalculations.riskMitigationValue) * 
           baseCalculations.marketMultiplier * 
           routeMultiplier
        )
        ...
        return {
          ...
          tariffSavings: Math.round(routeSpecificSavings * routeMultiplier),
          processSavings: Math.round(baseCalculations.processSavings * routeMultiplier),
          breakdown: {
            tariffElimination: Math.round(routeSpecificSavings * routeMultiplier),
            processOptimization: Math.round(baseCalculations.processSavings * routeMultiplier),
            timeToMarket: Math.round(baseCalculations.timeValueSavings * routeMultiplier),
            riskMitigation: Math.round(baseCalculations.riskMitigationValue * routeMultiplier)
          },
          confidence: this.calculateSavingsConfidence(businessProfile, route.type || 'triangle'),
          roi: this.calculateQuickROI(totalSavings, businessProfile)
        }
      })
      ...
      return await Promise.all(routes.map(async (route) => {
        return this.calculateRouteSavings({
          route: route.route,
          importVolume: batchContext.importVolume,
          products: batchContext.products,
          businessProfile: batchContext.businessProfile
        })
      }))
    }
  }
  ...
  /**
   * Quick ROI calculation for batch processing
   */
  calculateQuickROI(annualSavings, businessProfile) {
    const implementationCosts = this.calculateImplementationCosts(businessProfile)
    const roi = ((annualSavings - implementationCosts.ongoing) / implementationCosts.initial) * 100
    return Math.round(roi * this.getIndustryROIMultiplier(businessProfile.businessType))
  }

  /**
   * Get route type multiplier for different routing strategies
   */
  getRouteTypeMultiplier(routeType, usmcaTariff = 0) {
    const multipliers = {
      'TRIANGLE_USMCA': 1.0, // Maximum savings
      'DIRECT': 0.3, // Lower savings, direct route
      'ALTERNATIVE': 0.8 // Moderate savings
    }

    // USMCA routes get maximum benefit
    if (usmcaTariff === 0) {
      return multipliers['TRIANGLE_USMCA']
    }

    return multipliers[routeType] || 0.85
  }

  /**
   * Individual route savings calculation (kept for compatibility and fallback)
   */
  async calculateRouteSavings(context) {
    const { route, importVolume, products, businessProfile } = context
    
    const annualSavings = this.calculateAnnualSavings(
      businessProfile, 
      'optimal', 
      products?.[0]?.complexity || 'medium'
    )

    return {
      route,
      annualSavings: annualSavings.totalAnnualSavings,
      breakdown: annualSavings.breakdown,
      confidence: annualSavings.confidenceLevel,
      roi: this.calculateROI(annualSavings.totalAnnualSavings, businessProfile).roi
    }
  }
}
