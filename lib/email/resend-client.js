...
export async function sendEmail({ to, subject, html, from }) {
  const fromEmail = from || process.env.EMAIL_FROM || 'noreply@triangle-trade-intel.site';
...
export async function sendContactFormNotification(formData) {
  const { name, email, company, phone, message, industry, lead_source } = formData;
...
<body style="margin: 0; padding: 0; font-family: 'Arial', sans-serif; background-color: #f4f4f4;">
    ...
    <tr>
      <td align="center">
        <table role="presentation" cellpadding="0" cellspacing="0" width="600" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          <!-- Header -->
          <tr>
            <td style="background: linear-gradient(135deg, #134169 0%, #2563eb 100%); padding: 30px; text-align: center;">
          ...
          <tr>
            <td style="padding: 30px;">
              <p style="margin: 0 0 20px; color: #333333; font-size: 16px; line-height: 1.6;">
                A new contact form has been submitted on the Triangle Trade Intelligence platform.
              </p>

              <div style="background-color: #f8f9fa; border-left: 4px solid #2563eb; padding: 20px; margin: 25px 0; border-radius: 4px;">
                <h3 style="margin: 0 0 15px; color: #134169; font-size: 18px;">Contact Information:</h3>
                <p style="margin: 8px 0; color: #333333; font-size: 14px;"><strong>Name:</strong> ${name}</p>
                <p style="margin: 8px 0; color: #333333; font-size: 14px;"><strong>Email:</strong> <a href="mailto:${email}" style="color: #2563eb;">${email}</a></p>
                ${company ? `<p style="margin: 8px 0; color: #333333; font-size: 14px;"><strong>Company:</strong> ${company}</p>` : ''}
                ${phone ? `<p style="margin: 8px 0; color: #333333; font-size: 14px;"><strong>Phone:</strong> ${phone}</p>` : ''}
                ${industry ? `<p style="margin: 8px 0; color: #333333; font-size: 14px;"><strong>Category:</strong> ${industry}</p>` : ''}
                ${lead_source ? `<p style="margin: 8px 0; color: #333333; font-size: 14px;"><strong>Source:</strong> ${lead_source}</p>` : ''}
              </div>

              <div style="background-color: #fff8e1; border-left: 4px solid #f59e0b; padding: 20px; margin: 25px 0; border-radius: 4px;">
                <h3 style="margin: 0 0 15px; color: #92400e; font-size: 18px;">Message:</h3>
                <p style="margin: 0; color: #333333; font-size: 14px; line-height: 1.6; white-space: pre-wrap;">${message}</p>
              </div>

              <p style="margin: 25px 0 0; color: #666666; font-size: 13px; line-height: 1.6;">
                <strong>Next Steps:</strong><br/>
                1. Review the submission in the sales_prospects table<br/>
                2. Respond to the customer within 24 hours<br/>
                3. Update prospect status in admin dashboard
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="background-color: #f8f9fa; padding: 20px; text-align: center; border-top: 1px solid #e5e7eb;">
              <p style="margin: 0; color: #666666; font-size: 12px;">
                Triangle Trade Intelligence - Contact Form Notification
              </p>
              <p style="margin: 5px 0 0; color: #999999; font-size: 11px;">
  ...
  return await sendEmail({
    to: 'macproductions010@gmail.com',
    subject: `[${industry?.toUpperCase() || 'CONTACT'}] New Form Submission from ${name}`,
    html,
    from: 'Triangle Trade <onboarding@resend.dev>'
  });
}
