...
import { MexicoSourcingAgent } from '../agents/mexico-sourcing-agent.js';
...
CRITICAL POLICY EXPOSURE (2025): This company has Chinese-origin components subject to Section 301 tariffs even with USMCA qualification. Base MFN duties are eliminated, but Section 301 tariffs REMAIN - creating policy risk and ongoing cost burden. IMPORTANT: Determine the ACTUAL Section 301 rate from the USTR tariff list - rates vary by HS code (7.5%-50%, most electronics at 25%, some steel at 7.5%).` : ''}${deMinimisNote ? `

${deMinimisNote}` : ''}

PREFERENCE CRITERION: Determine which USMCA criterion applies (A/B/C/D) based on qualification method (${threshold.method}).
- Criterion A: All materials wholly obtained in USMCA territory (rare for complex products)
- Criterion B: RVC (Regional Value Content) rule met - ${threshold.rvc}% requirement satisfied
  ...
  strategic_alternatives: [
    {
      ...
      cost_impact: "Continue paying Section 301 tariffs (annual cost burden calculated above)",
    ...
    },
    {
      ...
      cost_impact: "CALCULATE based on user's industry: electronics 1-3%, automotive 2-4%, textiles 0-2%, aerospace 15-20%, machinery 3-5%. Apply percentage to trade_volume to get annual cost in dollars.",
      benefit: "Eliminates Section 301 exposure, increases RVC buffer, locks in preferential treatment",
      timeline: "CALCULATE based on product complexity: simple (consumer goods, textiles) = 4-8 weeks, standard (electronics, automotive parts) = 6-12 weeks, complex (machinery, medical devices) = 12-20 weeks, highly engineered (aerospace, defense) = 20-26 weeks",
      payback_months: "CALCULATE using formula: (one_time_setup_cost + annual_premium_cost) / (monthly_section_301_savings). DO NOT use placeholder number.",
 ...
 * REPLACES hardcoded +2% cost, 4-6 weeks timeline, 3-month payback
 *
 * @param {Object} formData - Form submission data
 * @param {Object} section301Exposure - { annual_cost_burden, affected_components }
 * @returns {Promise<Object>} Mexico sourcing analysis
 */
export async function calculateMexicoNearshoring(formData, section301Exposure) {
  const mexicoAgent = new MexicoSourcingAgent();

  try {
    // Get cost premium estimate from AI
    const costPremiumResult = await mexicoAgent.estimateMexicoCostPremium({
      industry_sector: formData.industry_sector,
      product_complexity: formData.product_complexity || 'medium',
      annual_trade_volume: formData.trade_volume || 0,
      current_origin: 'China'
    });

    // Get implementation timeline estimate from AI
    const timelineResult = await mexicoAgent.estimateImplementationTimeline({
      industry_sector: formData.industry_sector,
      product_complexity: formData.product_complexity || 'medium'
    });

    // Get setup cost estimate from AI
    const setupCost = await mexicoAgent.estimateSetupCost({
      industry_sector: formData.industry_sector,
      product_complexity: formData.product_complexity || 'medium',
      requires_tooling: formData.requires_custom_tooling || false
    });

    // Calculate payback period
    const paybackResult = mexicoAgent.calculatePaybackPeriod({
      current_tariff_annual_burden: section301Exposure.annual_cost_burden,
      mexico_cost_premium_percent: costPremiumResult.data?.premium_percent || 2.0,
      annual_trade_volume: formData.trade_volume || 0,
    ...
    return {
      success: true,
      cost_premium: costPremiumResult.data,
