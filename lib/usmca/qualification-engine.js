  ...
  strategic_alternatives: [
    {
      ...
      cost_impact: "Continue $${preCalculatedFinancials.section_301_exposure?.annual_cost_burden?.toLocaleString() || 0}/year Section 301 burden with ongoing policy risk",
      risk_profile: "Exposed to policy changes (rates can shift with 30-day notice from USTR)",
      timeline: "Immediate",
      recommendation: "Maintain current approach while monitoring USTR policy announcements"
    }${preCalculatedFinancials.section_301_exposure?.is_exposed ? `,
    {
      option: "Evaluate Mexico sourcing alternatives",
      cost_impact: "Typical sourcing premiums range 1-3% for electronics (varies by component type and supplier). Detailed cost analysis requires supplier quotes and RFQ process.",
      benefit: "Potential to eliminate $${preCalculatedFinancials.section_301_exposure?.annual_cost_burden?.toLocaleString() || 0}/year Section 301 exposure, increase RVC buffer, achieve policy insulation",
      timeline: "Varies significantly based on product complexity, supplier availability, and qualification requirements. Typical range: 6-16 weeks for supplier qualification and transition planning.",
      recommendation: "Consult with supply chain and procurement teams to evaluate Mexican supplier options, obtain detailed cost estimates, and assess implementation feasibility for your specific products and business processes."
    }` : ''}
  ],
  competitive_context: {
