...
export async function buildComprehensiveUSMCAPrompt(formData, componentRates = {}, preCalculatedFinancials = {}) {
  ...
  // ✅ AI will calculate labor credit dynamically based on actual manufacturing activities
...
CALCULATE the manufacturing labor credit percentage based on these activities:
${manufacturingProcess ? `- Actual processes: ${manufacturingProcess}` : '- No manufacturing process details provided (use industry average)'}
- Industry context: ${industry}
- Complexity: ${substantialTransformation ? 'High (substantial transformation)' : 'Standard (assembly)'}

Consider:
1. Direct labor (assembly, processing, testing, integration)
2. Overhead (facilities, supervision, quality control, engineering)
3. USMCA Article 4.5 guidelines (typically 10-25% for manufacturing)
4. Industry norms (electronics ~12-18%, automotive ~15-20%, textiles ~8-15%)

Return your calculated labor_credit_percentage as part of the RVC calculation.` : `Manufacturing in ${manufacturingLocation} (non-USMCA country) = 0% labor credit.`}
