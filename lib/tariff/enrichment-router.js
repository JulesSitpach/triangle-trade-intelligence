...
[
  ...
  async enrichFromDatabase(component, destination_country, hs_code = null, context = {}) {
    try {
      ...
      // ✅ CRITICAL FIX: Select correct tariff rate based on origin country
      // China-origin: Use Column 2 rate (non-WTO, e.g., 35% for microprocessor)
      // Other origins: Use MFN rate (WTO, e.g., 0% for microprocessor)
      let mfn_rate = 0;

      if (component.origin_country && component.origin_country.toUpperCase() === 'CN') {
        // China-origin goods: Use Column 2 rate (non-WTO tariff)
        mfn_rate = parseFloat(data.column_2_ad_val_rate) || parseFloat(data.mfn_ad_val_rate) || 0;
        logInfo('Using Column 2 rate (China-origin goods)', {
          hs_code,
          column_2_rate: parseFloat(data.column_2_ad_val_rate) || 0,
          mfn_rate: parseFloat(data.mfn_ad_val_rate) || 0,
          selected_rate: mfn_rate
        });
      } else {
        // WTO countries: Use standard MFN rate
        mfn_rate = parseFloat(data.mfn_ad_val_rate) || 0;
      }
