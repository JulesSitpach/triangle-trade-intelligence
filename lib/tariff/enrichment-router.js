...
export class EnrichmentRouter {
  ...
  async batchAITariffLookup(components, destination_country, product_description, strategy) {
    ...
    const prompt = `Research tariff rates for ${destination_country} destination for the following ${components.length} components:
...
[
  {
    "component_index": 1,
    "hs_code": "...",
    "base_mfn_rate": 0.0,         // ✅ ISSUE #2 FIX: Base MFN without policy adjustments
    "section_301": 0.0,           // Policy tariff for Chinese goods to USA
    ...
    "policy_adjustments": ["Section 301 for Chinese origin", ...]  // List of applicable policies
  }
]`;
    ...
    try {
      ...
      return results.map((result, index) => ({
        ...
        mfn_rate: parseFloat(result.mfn_rate) || 0,
        base_mfn_rate: parseFloat(result.base_mfn_rate || result.mfn_rate) || 0,
        section_301: parseFloat(result.section_301) || 0,
        section_232: parseFloat(result.section_232) || 0,
        total_rate: parseFloat(result.total_rate || result.mfn_rate) || 0,
  ...
  async enrichComponent(component, destination_country, product_description, hs_code = null, context = {}) {
    try {
      ...
      logInfo('Enrichment router - routing request', {
        component_origin: component.country,
  ...
  async checkCache(component, destination_country, hs_code, ttl_hours) {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('origin_country', component.country)
