...
[
  {
    ...
    try {
      ...
      return await Promise.all(results.map(async (result, aiIndex) => {
        ...
        let baseMfn = normalizeRateToDecimal(result.base_mfn_rate || result.mfn_rate || 0);
        let section301 = normalizeRateToDecimal(result.section_301 || 0);
        const section232 = normalizeRateToDecimal(result.section_232 || 0);
        ...
        if ((component.origin_country === 'CN' || component.origin_country === 'China') && destination_country === 'US') {
          try {
            ...
            if (section301Result.success && section301Result.data.applicable) {
              ...
              if (validation.valid) {
                section301 = validatedRate;
                console.log(`✅ Section 301 validated for ${result.hs_code}: ${(section301 * 100).toFixed(1)}% (List: ${section301Result.data.list})`);
              } else {
        ...
        const calculatedTotalRate = calculateTotalRate(baseMfn, section301, section232);
        ...
        const savingsPercent = this.calculateSavingsPercentage(baseMfn, usmcaRate);
