...
import { VolatilityManager } from './volatility-manager.js';
import {
...
[
   ...
   *
   * ✅ ENHANCED: Now checks volatility tier before using database
   * Super-volatile combinations (China semiconductors, etc.) bypass database entirely
   */
  async enrichFromDatabase(component, destination_country, hs_code = null, context = {}) {
    try {
      ...
      // ✅ NEW: Check volatility tier BEFORE database lookup
      console.log('🔍 [VOLATILITY-CHECK] Checking volatility BEFORE database lookup:', {
        hs_code,
      ...
      console.log('⚠️ [VOLATILITY-RESULT] Volatility tier determined:', {
        hs_code,
        origin: component.origin_country,
        destination: destination_country,
      ...
      // ✅ CRITICAL: If super-volatile, bypass database and force AI research
      if (volatilityTier.bypassDatabase) {
        console.log('🚨 [SUPER-VOLATILE] Bypassing database, forcing AI research!', {
          hs_code,
          origin: component.origin_country,
          destination: destination_country,
          tier: volatilityTier.tier,
        ...
        // Force AI lookup (24-hour cache only)
        return await this.enrichWithAI_24HrCache(
          component,
          destination_country,
          context.product_description || component.component_type,
          hs_code,
          {
            ...context,
      ...
      // ✅ CORRECT APPROACH: Use base MFN rate for ALL WTO countries (including China)
      ...
      //
      // NOTE: This database lookup is ONLY used for stable destinations (Mexico).
      // Volatile combinations (China → USA) bypass database via VolatilityManager.

      const mfn_rate = parseFloat(data.mfn_ad_val_rate) || 0;

      logInfo('Using base MFN rate (WTO member)', {
        hs_code,
        origin: component.origin_country,
        mfn_rate: mfn_rate,
        note: 'Policy tariffs (Section 301, etc.) added separately by AI for volatile destinations'
      });
  ...
  async checkCache(component, destination_country, hs_code, ttl_hours) {
    try {
      ...
      // ✅ CRITICAL: If super-volatile and bypassDatabase=true, ignore cache entirely
      if (volatilityTier.bypassDatabase) {
        logInfo('Super-volatile rate - ignoring cache, forcing fresh AI', {
          hs_code,
          origin: component.origin_country,
          destination: destination_country,
          tier: volatilityTier.tier
        });
        return null; // Force fresh AI lookup
      }

      // ✅ Use volatility-based TTL instead of destination-based TTL
      const effectiveTTL = Math.min(ttl_hours, volatilityTier.cacheTTL);

      logInfo('Checking cache with volatility-adjusted TTL', {
        hs_code,
        requested_ttl: ttl_hours,
        volatility_ttl: volatilityTier.cacheTTL,
      ...
      const { data, error } = await supabase
        ...
        .select('hs_code, hs_description, mfn_rate, base_mfn_rate, section_301, section_232, total_rate, usmca_rate, savings_percentage, ai_confidence, policy_adjustments, cached_at, expires_at')
      ...
      // ✅ ADDITIONAL CHECK: Validate cache age against volatility TTL
      const cacheAgeHours = (Date.now() - new Date(data.cached_at).getTime()) / (1000 * 60 * 60);

      if (cacheAgeHours > effectiveTTL) {
        logInfo('Cache entry exists but exceeds volatility TTL - forcing refresh', {
          hs_code,
          cache_age_hours: Math.round(cacheAgeHours),
          effective_ttl: effectiveTTL,
          volatility_tier: volatilityTier.tier
        });
        return null; // Cache too old for this volatility level
      }

      logInfo('Cache HIT - using cached data', {
        hs_code,
        cache_age_hours: Math.round(cacheAgeHours),
        volatility_tier: volatilityTier.tier
      });
  ...
  formatCachedResult(cached) {
    ...
    // ✅ CRITICAL: Calculate total_rate from all policy components
    const baseMfnRate = parseFloat(cached.base_mfn_rate) || parseFloat(cached.mfn_rate) || 0;
    const section301 = parseFloat(cached.section_301) || 0;
    const section232 = parseFloat(cached.section_232) || 0;
    ...
    return {
      ...
      mfn_rate: parseFloat(cached.mfn_rate) || baseMfnRate,
  ...
  calculateSavingsPercentage(mfn_rate, usmca_rate) {
    ...
    // ✅ FIX (Nov 20, 2025): Don't assume MFN=0 means USMCA=0
    // Edge case: Some products are duty-free under MFN but have tariffs under USMCA
    // Example: MFN=0%, USMCA=2.5% → Negative savings (you pay MORE under USMCA)

    // If MFN is 0, calculate absolute savings instead of percentage
    if (mfn === 0) {
      if (usmca === 0) {
        return 0;  // Both free, no savings
      }
      // MFN=0 but USMCA>0 means you pay MORE under USMCA (rare but possible)
      // Return negative to indicate USMCA is worse
      return -100;  // Indicate 100% worse (paying something vs nothing)
    }
