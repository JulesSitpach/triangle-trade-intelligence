...
// Initialize tariff research agent
...
- Section 301 Tariffs: Apply ~25% duty to Chinese-origin goods when destination is USA. Does NOT apply to Mexico or Canada origin.
...
✓ APPLY Section 301 (25%) when: Origin = "CN" or "China" AND Destination = "US"
...
[
  {
    ...
    try {
      ...
      return results.map((result, index) => {
        // Parse tariff components
        const baseMfn = parseFloat(result.base_mfn_rate || result.mfn_rate) || 0;
        const section301 = parseFloat(result.section_301) || 0;
        const section232 = parseFloat(result.section_232) || 0;

        // ✅ CRITICAL: Calculate total_rate as sum of all tariff components
        // DO NOT trust AI to calculate this correctly - compute it ourselves
        const calculatedTotalRate = baseMfn + section301 + section232;
        const reportedTotalRate = parseFloat(result.total_rate) || 0;

        return {
          hs_code: result.hs_code,
          hs_description: result.description,
          mfn_rate: parseFloat(result.mfn_rate) || calculatedTotalRate || 0,
          base_mfn_rate: baseMfn,
          section_301: section301,
          section_232: section232,
          // ✅ Use calculated total, not AI-provided (AI may return base rate only)
          total_rate: calculatedTotalRate || reportedTotalRate || baseMfn || 0,
          usmca_rate: parseFloat(result.usmca_rate) || 0,
          savings_percentage: this.calculateSavingsPercentage(result.mfn_rate, result.usmca_rate),
  ...
  async storeInCache(component, destination_country, hs_code, aiResult, ttl_hours) {
    try {
      ...
      // Note: Database has no unique constraint on these columns, so we just insert
      // Multiple entries per HS code/destination are OK (cache TTL handles freshness)
