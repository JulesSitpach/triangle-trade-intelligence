        ...
        if ((component.origin_country === 'CN' || component.origin_country === 'China') && destination_country === 'US') {
          try {
            ...
            if (section301Result.success && section301Result.data.applicable) {
              // Use actual Section 301 rate from agent
              section301 = section301Result.data.rate;
              console.log(`✅ Section 301 validated for ${result.hs_code}: ${section301 * 100}% (List: ${section301Result.data.list})`);
            }
          } catch (error) {
