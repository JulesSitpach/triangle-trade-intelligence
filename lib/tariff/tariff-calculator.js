 ...
 * @param {string} destination_country - Destination ('US', 'CA', 'MX') - REQUIRED, no default
...
export async function enrichComponentsWithTariffIntelligence(components, businessContext, destination_country) {
  // ✅ Fail loudly if destination_country missing (no silent 'US' default)
  if (!destination_country) {
    throw new Error('destination_country is required for component enrichment. Expected: US, CA, or MX');
  }
 ...
 * @param {string} destination_country - REQUIRED: US, CA, or MX (no default)
 */
export async function lookupBatchTariffRates(components, destination_country) {
  // ✅ Fail loudly if destination_country missing (no silent 'US' default)
  if (!destination_country) {
    throw new Error('destination_country is required for tariff lookup. Expected: US, CA, or MX');
  }
  const dbCachedRates = {};
  ...
  if (openRouterResult.success) {
    ...
    return await cacheBatchResults(openRouterResult.rates, uncachedComponents, cachedRates, destination_country);
  }
  ...
  if (anthropicResult.success) {
    ...
    return await cacheBatchResults(anthropicResult.rates, uncachedComponents, cachedRates, destination_country);
  }
 ...
 * @param {string} destination_country - REQUIRED: US, CA, or MX (no default)
 */
async function cacheBatchResults(freshRates, components, existingCache, destination_country) {
  // ✅ Fail loudly if destination_country missing (no silent 'US' default)
  if (!destination_country) {
    throw new Error('destination_country is required for cache results. Expected: US, CA, or MX');
  }

  // ✅ IMPORTANT: Await database save BEFORE returning (don't fire-and-forget)
  // This ensures data preparation is complete before AI analysis begins
  try {
    await saveTariffRatesToDatabase(freshRates, components, destination_country);
    ...
    // Still return merged cache even if save fails - use in-memory cache as fallback
  }
 ...
 * @param {string} destination_country - REQUIRED: US, CA, or MX (no default)
 */
export async function saveTariffRatesToDatabase(freshRates, components, destination_country) {
  // ✅ Fail loudly if destination_country missing (no silent 'US' default)
  if (!destination_country) {
    throw new Error('destination_country is required for saving tariff rates. Expected: US, CA, or MX');
  }
      ...
      savePromises.push(
        supabase
          ...
          .upsert({
            ...
            onConflict: 'hs_code,destination_country'  // ✅ FIXED: Use UPSERT instead of INSERT to handle duplicates
          })
          .then(({ error }) => {
            if (error) {
 ...
 * @param {string} component.destination_country - REQUIRED: US, CA, or MX
 */
export async function saveAIDataToDatabase(classificationResult, component) {
  // ✅ Fail loudly if destination_country missing (no silent 'US' default)
  if (!component.destination_country) {
    throw new Error('component.destination_country is required for database save. Expected: US, CA, or MX');
  }
  ...
  try {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        destination_country: component.destination_country,  // ✅ No fallback - validated above
        ...
        data_source: 'openrouter',
        expires_at: new Date(Date.now() + getCacheExpiration(component.destination_country)).toISOString(),  // ✅ Use actual destination, not hardcoded 'US'
