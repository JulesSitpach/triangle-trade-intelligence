...
export async function lookupHTSTariffRates(hsCode) {
  try {
    ...
    if (error) {
      await DevIssue.apiError('database_service', 'hts-lookup.lookupHTSTariffRates', error, {
        hsCode: normalizedCode,
        operation: 'SELECT',
        table: 'tariff_intelligence_master'
      });
    }
    ...
    if (!data) {
      await logDevIssue({
        ...
        message: `HS code not found in tariff_intelligence_master table`,
        data: {
          hsCode: normalizedCode,
          table: 'tariff_intelligence_master',
          expectedTable: 'tariff_intelligence_master'
        }
      });
    }
    ...
    await logDevIssue({
      ...
      severity: 'critical',
      component: 'database_service',
      message: 'Database connection error during HTS tariff lookup',
      data: {
        error: error.message,
        stack: error.stack,
        hsCode,
...
export function generateTariffChangeAlert(company, impactAnalysis) {
  return {
    ...
    financial_impact: {
      ...
      affected_trade_volume: company.trade_volume,
      impact_percentage: (Math.abs(impactAnalysis.total_annual_impact) / company.trade_volume) * 100
    },
