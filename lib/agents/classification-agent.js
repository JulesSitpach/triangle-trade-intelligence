...
Value Percentage: ${componentOrigins[0].value_percentage}%`
    ...
    // Extract primary material from description for validation
    const extractedMaterial = this.extractPrimaryMaterial(productDescription);
    const transformationNote = additionalContext.substantialTransformation && additionalContext.manufacturingProcess
      ? `\n\nTRANSFORMATION CONTEXT:\n✓ Substantial transformation confirmed\nManufacturing Process: ${additionalContext.manufacturingProcess}\n→ This component underwent significant processing - prioritize material-based classification`
      : '';

    const prompt = `Classify this component for US customs (HS code) using the CORRECT GRI hierarchy.

COMPONENT TO CLASSIFY:
"${productDescription}"
Origin: ${componentOrigins[0].origin_country}
Value: ${componentOrigins[0].value_percentage}% of product
Transformed: ${additionalContext.substantialTransformation ? 'YES' : 'NO'}${transformationNote}
...
Already classified (don't reuse):
${additionalContext.previouslyClassifiedComponents.map(c => `${c.hs_code}: ${c.description}`).join('\n')}
` : ''}

⚖️ CRITICAL: FOLLOW GRI HIERARCHY IN ORDER (DO NOT SKIP STEPS):

STEP 1 - MATERIAL DETERMINATION (MANDATORY):
Extract the PRIMARY material from the description above.
Material determines the chapter:
  ...
  • Electronics/Semiconductors → Chapter 85
  • Machinery/Mechanical → Chapter 84

STEP 2 - FORM/PROCESSING:
How is it manufactured? (cast, forged, machined, molded, extruded, stamped)
Physical form? (housing, bracket, fastener, wire, sheet, tube, assembly)

STEP 3 - FUNCTION (REFINES WITHIN MATERIAL CHAPTER ONLY):
Intended use refines the subheading WITHIN the correct material chapter.
Example: Aluminum mounting bracket → Chapter 76 (aluminum) → 7616.99 (other articles) → .50 (brackets)
DO NOT let function override material (e.g., "mounting bracket" does NOT force Chapter 73 if material is aluminum)

VALIDATION RULES:
❌ If description says "aluminum" but you chose Chapter 73 (steel) → REJECT and RETRY with Chapter 76
❌ If description says "steel" but you chose Chapter 76 (aluminum) → REJECT and RETRY with Chapter 72/73
❌ If material contradicts chapter → MAX 50% confidence + flag in explanation
✅ If material matches chapter → Normal confidence scoring

Return JSON:
{
  "hs_code": "8-digit HS code",
  "description": "Exact official HTS description",
  "confidence": 0-100,
  "primary_material": "aluminum|steel|plastic|copper|etc",
  "classification_methodology": "material_based|function_based",
  "gri_step_1_material": "What material did you identify?",
  "gri_step_2_form": "What form/processing?",
  "gri_step_3_function": "What function (if applicable)?",
  "material_chapter_match": true|false,
  "audit_risk": "low|medium|high",
  "legal_basis": "GRI Rule applied (e.g., GRI 1: Material classification)",
  "explanation": "2-3 sentences explaining classification reasoning with GRI hierarchy",
  "usmca_qualification": "likely|possible|unlikely",
  "alternative_codes": [
    {"hs_code": "alternative 8-digit code", "confidence": 0-100, "reason": "why this alternative applies"},
    {"hs_code": "another alternative", "confidence": 0-100, "reason": "why this applies"}
  ]
}`;
    ...
    if (result.success && result.data.hs_code) {
      // ✅ NEW (Nov 10, 2025): Validate material-chapter consistency
  ...
  validateMaterialChapterMatch(hsCode, description, aiMaterial, aiMatch) {
    ...
    const materialChapterMap = {
      ...
      'wood': ['44'],
      'electronic': ['85'],
      'composite': ['39', '68', '70'] // Varies by type
    };
    ...
    if (validChapters.length === 0) {
      return { valid: true, message: `Unknown material mapping for "${primaryMaterial}"` };
    }

    if (!validChapters.includes(chapter)) {
      return {
        valid: false,
        warning: `Material "${primaryMaterial}" should use Chapter ${validChapters.join('/')}, but HS code ${hsCode} uses Chapter ${chapter}`,
