...
This business owner is counting on you. Be thorough, be accurate, be honest about confidence levels.`;
    ...
    const toolHandler = async (name, args) => {
      ...
      if (name === 'search_usitc_api') {
        const { USITCApiService } = await import('../services/usitc-api-service.js');
        ...
        // USITC API only uses keywords for search, but we pass full context back to AI
        const usitcResults = await USITCApiService.searchHTS(args.keywords);

        // Add component analysis to results so AI can use it for matching
        if (usitcResults.success && args.component_analysis) {
          usitcResults.component_context = args.component_analysis;
          usitcResults.matching_instructions = 'Use the component_context to pick the code that best matches the material, form, and processing level analyzed in Step 1.';
        }
    ...
    if (result.success && result.data.hs_code) {
      ...
      if (!verification.found) {
        ...
        // Update explanation text to reflect adjusted confidence
        if (result.data.explanation && originalConfidence > 70) {
          result.data.explanation = result.data.explanation.replace(
            /Confidence \d+%/g,
            `Confidence ${result.data.confidence}%`
          );
          result.data.explanation += ` (Note: Confidence reduced from ${originalConfidence}% to ${result.data.confidence}% because HS code not found in database - requires USITC verification)`;
        }
      }
