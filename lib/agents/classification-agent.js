...
Value Percentage: ${componentOrigins[0].value_percentage}%`
    ...
    const prompt = `Classify this component for import into the United States.

Component: "${productDescription}"
Origin: ${componentOrigins[0].origin_country}
Destination: United States
Value: ${componentOrigins[0].value_percentage}% of final product

Final Product: ${additionalContext.overallProduct} (${additionalContext.industryContext})
Manufacturing: ${additionalContext.manufacturingProcess || 'Not specified'}
${additionalContext.substantialTransformation ? 'Substantial transformation: Yes' : ''}${additionalContext.previouslyClassifiedComponents && additionalContext.previouslyClassifiedComponents.length > 0 ? `
Other components: ${additionalContext.previouslyClassifiedComponents.map(c => `${c.hs_code}`).join(', ')}` : ''}

You have access to the US HTS database. Use the search_us_hts_codes tool to:
1. Search by keywords to find relevant chapters
2. Verify your chosen code exists in the database
3. Find valid alternatives if your first choice doesn't exist

Return the 8-digit US HTS code that you've verified exists in the database.

JSON:
{
  "hs_code": "8-digit HS code",
  "description": "Official HTS description",
is the   "confidence": 0-100,
  "primary_material": "material or 'functional'",
  "explanation": "Brief reasoning",
  "alternative_codes": [
    {"hs_code": "...", "confidence": 0-100, "reason": "..."}
  ]
}`;

    // ✅ Provide database search tools to AI
    const tools = [
      {
        type: 'function',
        function: {
          name: 'search_us_hts_codes',
          description: 'Search US Harmonized Tariff Schedule database for valid codes. Use this to verify codes exist or find valid alternatives.',
          parameters: {
            type: 'object',
            properties: {
              chapter: {
                type: 'string',
                description: 'First 2 digits of HS code (e.g., "85" for electrical machinery)'
              },
              heading: {
                type: 'string',
                description: 'First 4 digits of HS code (e.g., "8534" for printed circuits)'
              },
              keywords: {
                type: 'string',
                description: 'Keywords to search in code descriptions (e.g., "printed circuits", "aluminum")'
              },
              exact_code: {
                type: 'string',
                description: '8-digit code to verify exists (e.g., "85340000")'
              }
            }
          }
        }
    ...
    const toolHandler = async (toolName, args) => {
      if (toolName === 'search_us_hts_codes') {
    ...
    if (result.success && result.data.hs_code) {
      // ✅ NEW (Nov 10, 2025): Verify HS code exists in database
      const codeVerification = await this.verifyHSCodeExists(result.data.hs_code);

      if (!codeVerification.exists) {
        console.warn(`⚠️ [CLASSIFICATION] HS code ${result.data.hs_code} not found in database`);

        if (codeVerification.validAlternative) {
          console.log(`✅ [CLASSIFICATION] Using valid alternative: ${codeVerification.validAlternative.hts8}`);

          // Replace with valid broader code
          result.data.hs_code = codeVerification.validAlternative.hts8;
          result.data.description = codeVerification.validAlternative.brief_description;
          result.data.confidence = Math.min(result.data.confidence || 70, 70); // Cap at 70% for broader code
          result.data.explanation = `⚠️ Original code not found in US HTS. Using broader valid code: ${codeVerification.validAlternative.hts8}\n\n${result.data.explanation}`;
          result.data.code_verification = 'substituted_broader_code';
        } else {
          // No valid alternative found - flag for manual review
          result.data.confidence = Math.min(result.data.confidence || 40, 40);
          result.data.explanation = `⚠️ WARNING: HS code ${result.data.hs_code} not found in US HTS database. Manual verification required.\n\n${result.data.explanation}`;
          result.data.code_verification = 'not_found_needs_verification';
        }
      } else {
        result.data.code_verification = 'verified_in_database';
      }
  ...
  /**
   * Verify HS code exists in database and find valid alternative if not
   * @private
   */
  async verifyHSCodeExists(hsCode) {
    try {
      ...
      if (exactMatch) {
        return { exists: true, code: exactMatch };
      }

      // Code doesn't exist - find broader valid alternative
      // Try heading (first 4 digits + zeros)
      const heading = normalized.substring(0, 4) + '0000';
      const { data: headingMatch } = await this.supabase
        .from('tariff_intelligence_master')
        .select('hts8, brief_description')
        .eq('hts8', heading)
        .single();
      ...
      if (headingMatch) {
        return {
          exists: false,
          validAlternative: headingMatch,
          reason: 'Used 4-digit heading code (broader classification)'
        };
      }

      // Try subheading (first 6 digits + zeros)
      const subheading = normalized.substring(0, 6) + '00';
      const { data: subheadingMatch } = await this.supabase
        .from('tariff_intelligence_master')
        .select('hts8, brief_description')
        .eq('hts8', subheading)
        .single();
      ...
      if (subheadingMatch) {
        return {
          exists: false,
          validAlternative: subheadingMatch,
          reason: 'Used 6-digit subheading code (broader classification)'
        };
      }

      // No valid alternative found
      return { exists: false, validAlternative: null };

    } catch (error) {
      console.error('[ClassificationAgent] Code verification error:', error.message);
      // On error, assume code is valid (don't block classification)
      return { exists: true };
    }
  }
