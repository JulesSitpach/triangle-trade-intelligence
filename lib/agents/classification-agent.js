...
IMPORTANT: You MUST use the search_us_hts_codes tool before responding. Do not guess codes.

Required process:
1. Call search_us_hts_codes with keywords to find the chapter (e.g., "printed circuit")
2. Call search_us_hts_codes with exact_code to verify your top choice exists
3. Call search_us_hts_codes to find 2-3 alternative codes
4. Only then return your verified 8-digit US HTS code

Do NOT return a code without first verifying it exists in the database using the tool.
...
JSON:
{
  ...
  "alternative_codes": [
    ...
    // ✅ Try with tools first, fallback to no tools if OpenRouter doesn't support it
    let result;
    try {
      ...
      // If tools are not supported, try without them
      if (error.message && error.message.includes('tools')) {
        console.warn('[ClassificationAgent] Tools not supported by model, falling back to no-tools classification');
        result = await this.execute(prompt, {
          productDescription,
          componentOrigins,
          temperature: 0.3
    ...
    if (result.success && result.data.hs_code) {
      // ✅ Log if AI used database tools
      if (result.usedTools) {
        console.log(`✅ [CLASSIFICATION] AI verified code using database tools`);
      } else {
        // ⚠️ Tools not used - quick verification check
        console.warn(`⚠️ [CLASSIFICATION] Tools not used - performing quick verification`);
        const verification = await this.searchHTSDatabase({ exact_code: result.data.hs_code });
        if (!verification.found) {
          console.warn(`⚠️ [CLASSIFICATION] Code ${result.data.hs_code} not found - suggesting lower confidence`);
          result.data.confidence = Math.min(result.data.confidence || 60, 60);
          result.data.explanation = `⚠️ Note: This code needs verification. ${result.data.explanation || ''}`;
        }
      }
