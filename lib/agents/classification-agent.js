...
1. PRIMARY MATERIAL: What is the main material?
   - Identify the predominant material composition
   - Material usually determines the chapter (e.g., base metals vs plastics vs textiles)

2. PRIMARY FORM: What physical state is this component in? (CRITICAL)
   - Raw material / unwrought → Typically .01/.02 subheadings
   - Waste / scrap / off-cuts → Typically .30 subheadings ⚠️ ONLY if explicitly waste/scrap
   - Worked forms (bars/rods/wire/forgings/castings) → Typically .40-.80 subheadings
   - Finished articles / manufactured parts → Typically .90+ subheadings
   - Semi-finished forms → Depends on processing level

3. PROCESSING LEVEL: How much manufacturing has been applied?
   - Heavily processed (forged, heat-treated, machined, coated) = FINISHED/WORKED forms
   - Lightly processed (cut to size, basic shaping) = SEMI-FINISHED forms
   - Unprocessed (as-extracted, as-produced) = RAW MATERIAL
   - Manufacturing waste (off-cuts, turnings, damaged) = WASTE/SCRAP ⚠️ Only if truly waste

4. PRIMARY FUNCTION: What does this component do? (Consider LAST, after form/material)
   - Functional components (machinery, electronics, instruments) often have specific chapters
   - Structural components typically classified by material
   - Classification is by WHAT IT IS (form + material), not end-use application
