...
Value Percentage: ${componentOrigins[0].value_percentage}%`
    ...
    const prompt = `Classify this component for US customs using the Harmonized Tariff Schedule.
...
Substantial Transformation: ${additionalContext.substantialTransformation ? 'Yes' : 'No'}
${additionalContext.previouslyClassifiedComponents && additionalContext.previouslyClassifiedComponents.length > 0 ? `
Previously Classified Components:
${additionalContext.previouslyClassifiedComponents.map(c => `  ${c.hs_code}: ${c.description}`).join('\n')}

NOTE: Each component is unique - classify independently, don't reuse chapters from previous components.` : ''}

HTS Classification Rules:
1. Read chapter/heading descriptions carefully - they contain critical exclusions
2. Match keywords: "without" vs "with", "not assembled" vs "assembled", "unmounted" vs "mounted"
3. HTS is hierarchical: Chapter (2 digits) → Heading (4 digits) → Subheading (6 digits) → Full code (8 digits)
4. The rightmost digits are statistical suffixes - only use them if you know the exact code
5. When uncertain about the full 8-digit code, use the broader 6-digit subheading and pad with zeros
6. A component's ACTUAL STATE determines its code - read the component description literally

Confidence Scoring:
- 90-100%: You know the exact 8-digit code with certainty
- 70-89%: Confident in chapter/heading, used broader code (padded with zeros)
- 50-69%: Multiple chapters could apply, chose most likely
- Below 50%: Insufficient information to classify accurately
