...
export class ClassificationAgent extends BaseAgent {
  constructor() {
    super({
      ...
      maxTokens: 1200  // Optimized: PRIMARY + ALTERNATIVE classifications with legal basis (~700 tokens typical, 1200 provides headroom)
    });
  ...
  async suggestHSCode(productDescription, componentOrigins = [], additionalContext = {}) {
    ...
    // ✅ NEW (Nov 6, 2025): Check classification cache first (permanent cache - classifications don't change)
    ...
    if (cachedClassification) {
      ...
      return {
        ...
        data: {
          ...
          confidence: cachedClassification.confidence_score * 100, // Convert 0.85 → 85
    ...
    console.log('⏳ [CLASSIFICATION CACHE MISS] Making fresh AI classification call...');
...
- **PRIMARY**: 8517.79.00 (Phone parts) - 0% MFN
  ...
  - Audit risk: LOW ✅
  - USMCA value: No savings (already duty-free)

- **ALTERNATIVE**: 7616.99.50 (Aluminum articles) - Research actual MFN rate
  - Legal basis: Material composition argument
  - Audit risk: HIGH ⚠️ (CBP may challenge/overturn)
  - USMCA value: Calculate based on component value * (MFN rate - USMCA rate)
