...
FALLBACK BEHAVIOR:
- If search_usitc_api fails (returns empty) → Use your training data to find HS code
- If search_database returns results AND you found code via training → Compare those 2
- If BOTH tools return results → Compare and pick best match
- If BOTH tools return empty → Use training data best-guess (confidence 40-50%)
...
This business owner is counting on you. Be thorough, be accurate, be honest about confidence levels.`;
    ...
    const toolHandler = async (name, args) => {
      ...
      if (name === 'search_usitc_api') {
        // AI research to find HS code based on synthesized analysis
        console.log('🔍 [AI RESEARCH] Synthesized component analysis:', {
        ...
        try {
          // Build full description from synthesized analysis
          const fullDescription = `
Material: ${args.component_analysis?.material || 'Not specified'}
Function: ${args.component_analysis?.function || 'Not specified'}
Form: ${args.component_analysis?.form || 'Not specified'}
Processing: ${args.component_analysis?.processing || 'Not specified'}
Specifications: ${args.component_analysis?.specifications || 'Not specified'}
Keywords: ${args.keywords}`.trim();

          console.log('🤖 [AI RESEARCH] Researching HS code...');

          // Use BaseAgent for AI research (has OpenRouter → Anthropic fallback)
          const researchAgent = new BaseAgent({
            model: 'anthropic/claude-haiku-4.5',
            maxTokens: 600
          });

          const researchPrompt = `Find the correct 8-digit HTS code for this component:
...
{
  "hts8": "85423100",
  "description": "Brief official HTS description",
  "confidence": 0.85
}`;

          const researchResult = await researchAgent.execute(researchPrompt, { temperature: 0.1 });

          if (researchResult.success && researchResult.data?.hts8) {
            console.log(`✅ [AI RESEARCH] Found ${researchResult.data.hts8} (${Math.round(researchResult.data.confidence * 100)}% confidence)`);

            return {
              success: true,
              results: [{
                code: researchResult.data.hts8,
                description: researchResult.data.description,
                confidence: Math.round(researchResult.data.confidence * 100),
                source: 'ai_research'
              }],
              message: 'AI research completed'
            };
          } else {
            console.warn('⚠️ [AI RESEARCH] No code found - AI will use training data');
            return {
              success: false,
              results: [],
              message: 'AI research failed - will use training data fallback'
            };
          }
          ...
          return {
            success: false,
            results: [],
            message: `AI research error: ${error.message} - will use training data fallback`
          };
        }
      }
      ...
      if (!dbVerification.found) {
        ...
        try {
          ...
          if (verification) {
            ...
            // ALWAYS use verified confidence (external verification is authoritative)
            // Our database being incomplete ≠ AI made a mistake
            const originalConfidence = result.data.confidence || 70;
            const verifiedConfidence = Math.round(verification.confidence * 100);

            // Update confidence to verified value (whether higher or lower)
            ...
            } else if (verifiedConfidence < originalConfidence) {
              result.data.explanation += ` (Confidence adjusted to ${verifiedConfidence}% based on ${verification.data_source} verification)`;
            } else {
              result.data.explanation += ` (Verified via ${verification.data_source})`;
            }
