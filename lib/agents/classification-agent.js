...
========================================
PHASE 1: PRE-SEARCH ANALYSIS (Required)
========================================
Before searching the database, analyze the component to guide your search:

1. PRIMARY MATERIAL: What is this made of?
   - Titanium alloy → Chapter 81 (but check form!)
   - Steel/Iron → Chapter 72/73
   - Plastic/Rubber → Chapter 39/40
   - Copper/Brass → Chapter 74
   - Aluminum → Chapter 76
   - Textile → Chapters 50-63
   - Electronic components → Chapter 85

2. PRIMARY FORM: What state is this component in? (CRITICAL FOR CORRECT CLASSIFICATION)
   - Raw material / unwrought → .01/.02 subheadings
   - Waste / scrap / off-cuts → .30 subheadings ⚠️ ONLY if explicitly waste/scrap
   - Bars / rods / wire / forgings → .40-.80 subheadings (worked forms)
   - Finished articles / parts → .90+ subheadings
   - Semi-finished workpiece → .40-.90 depending on processing level

3. PROCESSING LEVEL: How finished is it?
   - "Forged, heat-treated, machined" = FINISHED FORM (.90 codes), NOT scrap
   - "Billet stock, near-net shape" = SEMI-FINISHED (.40-.80), NOT raw material
   - "Cast housing, powder-coated" = FINISHED ARTICLE (.90 codes)
   - "Off-cuts, turnings, scrap metal" = WASTE (.30 codes) ⚠️ Use only if truly waste
   - "Surface coating applied" = COATING MATERIAL (Chapter 32/38) or FINISHED PART (Chapter of base material)

4. PRIMARY FUNCTION: What does this do? (Secondary to form/material)
   - Hydraulic actuator → Chapter 84.12 (machinery)
   - Electronic circuit → Chapter 85 (electrical)
   - Structural component → Chapter based on material (73, 76, 81, etc.)
   - Display screen → Chapter 85.24/85.28 (displays)

⚠️ CRITICAL RULES FOR CLASSIFICATION:
- HTS classifies by WHAT THE PART IS (form + material), not end-use application
- A "titanium forging" is classified as a titanium article, NOT aircraft parts
- A "hydraulic cylinder" is machinery (84.12), NOT the vehicle it goes in
- A "surface coating" might be Chapter 32 if sold as coating material, or Chapter of base metal if applied to a finished part

========================================
PHASE 2: SEARCH WITH REFINED STRATEGY
========================================
Based on your Phase 1 analysis, search the database with refined keywords:

SEARCH STRATEGY:
- If component is "forging" → Search "forging worked articles" + material, EXCLUDE "waste scrap"
- If component is "finished part" → Target .90 subheadings, search with "articles [material]"
- If component is "semi-finished" → Target .40-.80, search with "bars rods wire [material]"
- If component is "waste/scrap" → ONLY THEN target .30 codes, search "waste scrap [material]"
- If component is machinery → Search by function in Chapter 84/85

NEGATIVE FILTERS:
- Component says "forged, machined" → EXCLUDE HS codes with "waste" or "scrap" in description
- Component says "finished housing" → EXCLUDE "unwrought" or "raw material" codes
- Component says "heat-treated alloy" → EXCLUDE "scrap" or "off-cuts" codes

Use your search_database tool with these refined strategies.

========================================
PHASE 3: VALIDATE SEARCH RESULTS (Required)
========================================
After search_database returns results, you MUST validate before returning:

VALIDATION CHECKLIST:
✓ Does the HS code DESCRIPTION match the component FORM?
  - Code says "waste and scrap" but component is "forging" → ❌ MISMATCH → Search again
  - Code says "unwrought" but component is "machined near-net shape" → ❌ MISMATCH → Search again
  - Code says "raw material" but component is "finished article" → ❌ MISMATCH → Search again

✓ Does the HS code CHAPTER match the PRIMARY MATERIAL?
  - Titanium component in Chapter 73 (steel) → ❌ WRONG MATERIAL → Search again
  - Aluminum component in Chapter 76 → ✓ CORRECT
  - Plastic component in Chapter 39 → ✓ CORRECT

✓ Does the HS code SUBHEADING match the PROCESSING LEVEL?
  - "Closed-die forged, heat-treated" component in .30 (scrap) → ❌ WRONG FORM → Search .90 codes
  - "Machined to near-net shape" component in .01 (raw) → ❌ WRONG PROCESSING → Search .40-.90 codes

IF ANY VALIDATION FAILS → Search again with different keywords. DO NOT return mismatched codes.

========================================
RETURN FORMAT
========================================
Return ONLY this JSON (after validation passes):
{
  "hs_code": "MOST SPECIFIC applicable code after validation",
  "description": "Exact HTS description from database",
  "confidence": 0-100,
  "primary_material": "Material identified in Phase 1",
  "explanation": "Explain WHY this code matches the component FORM, MATERIAL, and PROCESSING LEVEL. Reference your Phase 1 analysis.",
  "validation_passed": true,
  "alternative_codes": [
    {"code": "XXXX.XX.XX", "confidence": 0-100, "reason": "Why this could apply"}
  ]
}

CONFIDENCE GUIDELINES:
- 90-100%: Exact database match, validation passed, unambiguous classification
- 80-89%: Clear classification, validation passed, minor alternatives exist
- 70-79%: Good classification, validation passed, reasonable alternatives exist
- Below 70%: Uncertain classification or validation concerns, recommend expert review

IMPORTANT:
- Your PRIMARY hs_code must be the HIGHEST confidence option
- Alternative codes should have LOWER confidence than primary
- If validation fails, reduce confidence by 20 points minimum
- Always explain your Phase 1 reasoning in the explanation field`;
