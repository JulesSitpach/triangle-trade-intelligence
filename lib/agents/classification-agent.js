...
• What is this component MADE OF? (material determines chapter)
  ...
  If component has multiple materials (e.g., "steel bracket with rubber gasket"):
  → HTS classification determined by PRIMARY MATERIAL BY WEIGHT (usually 50%+ by mass)
  → UNLESS the component's FUNCTION depends on one material (e.g., "rubber seal with metal housing" → classify by rubber if sealing is the function)
  → When unsure: Search for composite/multi-material codes in relevant chapter
  → Mark confidence "medium" and note material composition in justification
...
1. MATERIAL CHECK
   ❓ Does the HTS description mention the PRIMARY MATERIAL you identified in Step 1?
      ...
      ❌ NO → REJECT, search with material keyword (e.g., "titanium", "plastic", "steel")

2. FORM CHECK
   ❓ Does the HTS description match the FORM you identified?
      Your analysis: "worked/machined form" → HTS must say "articles", "worked", "parts" (NOT "scrap", "unwrought")
      Your analysis: "raw material" → HTS must say "unwrought", "ingots", "billets" (NOT "articles", "parts")
      Your analysis: "waste/scrap" → HTS must say "waste", "scrap", "turnings" (NOT "articles", "worked")
      ✅ MATCH → Proceed to Specificity Check
      ❌ MISMATCH → REJECT, search with form keyword (e.g., "worked titanium", "titanium articles")

3. SPECIFICITY CHECK
   ❓ If multiple codes seem valid, which is MOST SPECIFIC?
      • "Titanium articles, other" (8108.90) vs "Titanium forgings" (8108.30)
        → Choose the MORE SPECIFIC one (forgings if it's a forging)
      • If function is critical: "Hydraulic cylinders" (8412.30) vs "Other machinery parts" (8483.90)
        → Choose FUNCTIONAL category over generic "other parts"
...
Example reasoning:
"I found both 8108.30.00 (0.0%) and 7616.99.00 (2.5%). Chose 8108.30.00 because the primary material is titanium (Chapter 81), not aluminum (Chapter 76). Confidence: 85% (material clearly titanium, but form could be interpreted as either 'articles' or 'other worked forms')."
...
{
  "hs_code": "The code that passed validation and bias check",
  "description": "Exact description from database",
  "confidence": 0-100,
  "primary_material": "Material from your Step 1 analysis",
  "explanation": "Show your full reasoning: Step 1 analysis, Step 2 search strategy, Step 3 validation result, Step 4 bias check (if applicable)",
  "validation_passed": true,
  "bias_check_notes": "If duty-free: explain why you rejected higher-tariff alternatives, or null if not applicable",
  "alternative_codes": [
    {"code": "XXXX.XX.XX", "confidence": 0-100, "reason": "Why this could also apply"}
  ]
}
...
Your explanation should show your complete reasoning chain: "I analyzed this as [X], searched for [Y], validated that [Z] matches, and confirmed [duty-free choice is accurate because...]"
