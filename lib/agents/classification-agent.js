...
Classify this component as it crosses the border from ${componentOrigins[0].origin_country} (imported state, not finished product) to determine:
1. Import tariff rates (MFN vs USMCA preferential)
2. Regional Value Content for USMCA qualification${additionalContext.substantialTransformation && additionalContext.manufacturingProcess ? `\n3. Substantial transformation (${additionalContext.manufacturingProcess}) occurs AFTER import` : ''}

STRATEGIC ANALYSIS (Do this BEFORE searching):

Step 1 - Identify Core Characteristics:
- PRIMARY MATERIAL: What is it made of? (aluminum, steel, plastic, glass, copper, electronics, textiles, etc.)
- PRIMARY FUNCTION: What does it do? (housing/enclosure, circuit/PCB, display/screen, fastener, sensor, etc.)
- PRODUCT STATE: What form does it arrive in? (raw material, semi-finished, finished component, assembly)
...
- Display modules (LCD/LED/OLED) → Chapter 85 (8524 for displays, NOT 8534 for circuits)
- Glass articles → Chapter 70
- Copper articles → Chapter 74

Step 3 - Simplify Search Terms:
- Extract CORE material + function ONLY
- Remove unnecessary adjectives (anodized, powder-coated, tempered, capacitive, etc.)
- Examples:
  * "Die-cast aluminum housing with anodized finish, powder-coated matte black" → "aluminum housing"
  * "Capacitive touchscreen LCD display module, 7-inch, 1024x600" → "flat panel display" or "liquid crystal display"
  * "Multilayer printed circuit board with surface-mount components" → "printed circuit" or "PCB"

Step 4 - Execute Smart Search:
- Start with keywords + chapter
- If no results, try broader keywords
- If still no results, try chapter-only search

After finding candidate codes, SELECT THE MOST SPECIFIC applicable code as your PRIMARY hs_code.
Put less specific or alternative interpretations in alternative_codes with lower confidence.

Return ONLY this JSON:
{
  "hs_code": "MOST SPECIFIC applicable code",
  "description": "",
  "confidence": 0,
  "primary_material": "",
  "explanation": "",
  "alternative_codes": [
    {"code": "XXXX.XX.XX", "confidence": 0, "reason": "Why this could apply"}
  ]
}

CONFIDENCE GUIDELINES:
- 90-100%: Exact database match, unambiguous classification
- 80-89%: Clear classification with minor alternatives
- 70-79%: Good classification but reasonable alternatives exist
- Below 70%: Uncertain, recommend expert review

IMPORTANT: Your PRIMARY hs_code must be the HIGHEST confidence option. Alternative codes should have LOWER confidence than primary.`;
