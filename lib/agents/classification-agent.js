...
Value Percentage: ${componentOrigins[0].value_percentage}%`
    ...
    const prompt = `Classify this component for US Customs using the United States Harmonized Tariff Schedule (USITC).
...
Product Context:
${additionalContext.overallProduct} (${additionalContext.industryContext})
...
US HTS uses 8-digit codes. Structure: Chapter (2) → Heading (4) → Subheading (6) → Statistical suffix (8).
Do not use codes from other countries' tariff schedules (India, Mexico, etc.).
If you don't know the exact 8-digit US HTS code, use the 6-digit international code padded with zeros (e.g., 8534.00 becomes 85340000).
