...
Value Percentage: ${componentOrigins[0].value_percentage}%`
    ...
    const prompt = `Classify this component for US customs (HS code).

COMPONENT TO CLASSIFY:
"${productDescription}"
Origin: ${componentOrigins[0].origin_country}
Value: ${componentOrigins[0].value_percentage}% of product
Transformed: ${additionalContext.substantialTransformation ? 'YES' : 'NO'}
...
RULES:
1. Classify the COMPONENT by what it IS (not what product it goes into)
2. Function-first (CBP GRI Rule 1) unless it's a basic material
3. Use exact official HTS description (not paraphrase)
4. 8-digit HS code
...
Return JSON:
{
  ...
  "legal_basis": "Why this code (e.g., CBP GRI Rule 1)",
  "explanation": "2-3 sentences explaining classification reasoning",
