...
Manufacturing Process: ${additionalContext.manufacturingProcess}` : ''}${additionalContext.substantialTransformation ? `
Substantial Transformation: YES (significant value-added manufacturing)` : ''}${additionalContext.previouslyClassifiedComponents && additionalContext.previouslyClassifiedComponents.length > 0 ? `
Other components in this product: ${additionalContext.previouslyClassifiedComponents.map(c => `${c.description} (${c.hs_code})`).join(', ')}` : ''}
...
This component is PART OF the final product described above. Its classification should reflect its role in that specific end-use.
...
----------------------------------------------------
⚠️ CRITICAL: Consider the FINAL PRODUCT context above. A component's classification depends on what it's used for.

Ask yourself these questions IN ORDER:

• What is its PRIMARY FUNCTION IN THE FINAL PRODUCT?

  ❗ CONTEXT MATTERS:
  Same component name, different classifications based on end-use:
  → "Hydraulic cylinder" for aircraft → Chapter 88 (Aircraft parts)
  → "Hydraulic cylinder" for construction equipment → Chapter 84 (Machinery)
  → "Circuit board" for medical device → Chapter 90 (Medical instruments)
  → "Circuit board" for consumer electronics → Chapter 85 (Electrical machinery)

  ❗ FUNCTION-FIRST RULE:
  If component has a specific working function:
  → Check if it's specialized for the final product's industry (aircraft, medical, military)
  → If yes: Search for industry-specific chapter first
  → If generic: Search machinery/functional chapters (84, 85)

  If NO specific function (structural/raw material only):
  → Then classify by material (steel, titanium, plastic, etc.)

• What is this component MADE OF? (material determines chapter IF no specific function)
...
------------------------------------------------------
YOU MUST use the search_database tool now. Do NOT guess codes from memory.

Call search_database with keywords that match your analysis:
    ...
    const tools = [
      {
        ...
        function: {
          ...
          description: 'REQUIRED: Search HTS database with 12,118 codes. Call this FIRST before returning any HS code.',
          parameters: {
            type: 'object',
            properties: {
              keywords: {
