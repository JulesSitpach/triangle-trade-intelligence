export class BaseAgent {
  ...
  async execute(prompt, context = {}) {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        // ✅ Add tools if provided (for tool-enabled AI)
        if (context.tools && context.tools.length > 0) {
          requestBody.tools = context.tools;
          requestBody.tool_choice = 'auto'; // Let AI decide - but prompt will strongly encourage tool use
        }
        ...
        if (!response.ok) {
          ...
          console.error(`[${this.name}] OpenRouter API error response:`, errorBody);
        ...
        if (message.tool_calls && message.tool_calls.length > 0 && context.toolHandler) {
          console.log(`[${this.name}] 🔧 AI requested ${message.tool_calls.length} tool call(s)`);
          metrics.toolCalls += message.tool_calls.length;

          // ✅ Execute tool calls IN PARALLEL (not sequential)
          const toolResults = await Promise.all(
            message.tool_calls.map(async (toolCall) => {
              const toolName = toolCall.function.name;
              const toolArgs = JSON.parse(toolCall.function.arguments);

              console.log(`[${this.name}] 🔧 Executing tool: ${toolName}`, toolArgs);
              const result = await context.toolHandler(toolName, toolArgs);
          ...
          requestBody.messages.push(message); // AI's tool request
          requestBody.messages.push(...toolResults); // Tool results
          requestBody.messages.push({
            role: 'user',
            content: 'Now return the classification as valid JSON only.'
          });

          // 🔍 DEBUG: Log what we're sending back to OpenRouter
          console.log(`[${this.name}] 📤 Sending ${toolResults.length} tool results back to AI`);
          console.log(`[${this.name}] 📤 Tool result sample:`, JSON.stringify(toolResults[0]).substring(0, 200));
          ...
          if (!toolResponse.ok) {
            throw new Error(`OpenRouter API error after tools: ${toolResponse.status}`);
          }

          const toolData = await toolResponse.json();
          const responseText = toolData.choices[0].message.content;

          // 🔍 DEBUG: Log AI response after tool calls
          console.log(`[${this.name}] 📄 AI response after tools:`, responseText?.substring(0, 500));
          ...
          try {
            ...
            console.log(`[${this.name}] 📋 Parsed data:`, JSON.stringify(parsed).substring(0, 300));
          } catch (parseError) {
            metrics.parseErrors++;
          ...
          console.log(`[${this.name}] ✅ SUCCESS (with ${metrics.toolCalls} tool calls) - Duration: ${duration}ms`);
          return { success: true, data: parsed, metrics, usedTools: true };
        }
        ...
        // Special handling for rate limits (429 Too Many Requests, 529 Overloaded)
        if ((error.status === 429 || error.status === 529) && attempt < this.maxRetries) {
          const waitTime = Math.pow(2, attempt) * 1000;  // Exponential backoff
          console.log(`[${this.name}] 🚦 Rate limited (${error.status}), retrying in ${waitTime}ms...`);
  ...
  async executeWithAnthropic(prompt, systemPrompt, attempt) {
    ...
    const anthropicModelId = 'claude-haiku-4-20250514';
...
Response format: Always return valid JSON with your analysis.`;
  ...
  extractKeyValues(text) {
    try {
      ...
      // Extract common fields - match both camelCase (from AI) and snake_case (normalized)
      const hsCodeMatch = text.match(/"hs_code":\s*"([^"]+)"/) || text.match(/"hsCode":\s*"([^"]+)"/);
      if (hsCodeMatch) result.hs_code = hsCodeMatch[1];
  ...
  calculateConfidence(parsedResponse, context) {
    ...
    // Check for hs_code (snake_case) - this is now normalized
    const hsCode = parsedResponse.hs_code || parsedResponse.hsCode;
