  ...
  async execute(prompt, context = {}) {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        console.log(`[${this.name}] OpenRouter request:`, {
        ...
        console.error(`[${this.name}] OpenRouter attempt ${attempt} failed:`, error.message);

        // FALLBACK: Try Anthropic SDK if OpenRouter fails
        if (attempt === this.maxRetries) {
          ...
          try {
            ...
            console.error(`[${this.name}] Anthropic fallback also failed:`, fallbackError.message);
            return {
              success: false,
...
Response format: Always return valid JSON with your analysis.`;
  ...
  parseResponse(response) {
    try {
      // PRE-CLEAN the entire response first (before pattern matching)
      let preCleaned = response
        // Remove ALL control characters including problematic ones
        .replace(/[\u0000-\u001F\u007F-\u009F]/g, ' ')
        // Clean up line breaks and tabs thoroughly
        .replace(/\\n/g, ' ')
        .replace(/\\r/g, ' ')
        .replace(/\\t/g, ' ')
        .replace(/\n/g, ' ')
      ...
      // Now find the JSON block - try multiple patterns
      let jsonMatch = preCleaned.match(/\{[\s\S]*\}/);

      if (!jsonMatch) {
        // Try finding just the last complete JSON object
      ...
      if (!jsonMatch && preCleaned.includes('{')) {
        ...
        // Check if last field has an unclosed string value
        const lastQuoteIndex = incompleteJson.lastIndexOf('"');
        const lastColonIndex = incompleteJson.lastIndexOf(':');

        // If we have a colon after the last quote, we're in an unclosed string value
      ...
      if (jsonMatch) {
        let cleanedJson = jsonMatch[0]
          // Fix quotes in strings that contain line breaks
          .replace(/"\s*\n\s*"/g, ' ')
          .replace(/"\s*\r\s*"/g, ' ')
          ...
          // Fix multiple spaces
          .replace(/\s+/g, ' ')
          // Clean up quotes around colons
          .replace(/"\s*:\s*"/g, '": "')
          // Fix unescaped internal quotes
          .replace(/([^\\])"([^,:}\]]*)"([^,}\]])/g, '$1"$2\\"$3');

        // Additional safety: try to fix common JSON structure issues
        cleanedJson = this.fixJsonStructure(cleanedJson);
