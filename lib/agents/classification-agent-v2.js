...
Use reasoning, not checklists. If it doesn't make sense, don't return it.`;
  ...
  async searchDatabase(args) {
    ...
    try {
      // ✅ FIX (Nov 21): Use ILIKE instead of textSearch (websearch_to_tsquery is broken)
      // Split keywords and build ILIKE conditions for each word
      ...
      // Build ILIKE filter for all keywords (must match ALL keywords)
      // This mimics the behavior of full-text search but actually works
      ...
      // Client-side filtering for ALL keywords (since Supabase doesn't support chained ILIKE easily)
      if (data && keywordArray.length > 0) {
      ...
      console.log(`✅ [Search] Found ${data.length} results for: ${keywords}`);
