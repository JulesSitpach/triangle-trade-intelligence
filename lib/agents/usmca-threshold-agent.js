...
export class USMCAThresholdAgent extends BaseAgent {
  ...
  async fetchThresholdFromAI(hsCode, hsChapter, productCategory) {
    const prompt = `You are a USMCA trade policy expert with access to current treaty text and 2025 policy updates.
...
1. **USMCA Annex 4-B** (Product-Specific Rules of Origin) - PRIMARY SOURCE
2. **USMCA Chapter 4** (General Rules of Origin) - FALLBACK
3. **2025 USTR Federal Register notices** - Check for recent amendments

ANALYSIS STEPS:

1. **Check Annex 4-B** for product-specific rules by HS chapter:
   - Chapter 85 (Electronics): Usually 65% RVC (Transaction Value method)
   - Chapter 87 (Automotive): Usually 75% RVC (Net Cost method)
   - Chapter 61-63 (Textiles): Usually 55% RVC (Yarn Forward rule)
   - Chapter 84 (Machinery): Usually 60% RVC
   - Other chapters: Check Annex 4-B or use default

2. **Preference Criterion** (for USMCA Certificate Field 7):
   - "A": Good wholly obtained in territory
   - "B": Good produced entirely from originating materials (most common)
   - "C": Good produced from non-originating materials meeting change in tariff classification
   - "D": Good produced entirely in territory from non-originating materials

3. **Default Rule** (if not in Annex 4-B):
   - 62.5% RVC (Transaction Value method)
   - Article 4.2 (General Rule)
...
CONFIDENCE LEVELS:
- "high": Found in Annex 4-B with specific HS code
- "medium": Inferred from HS chapter rules
- "low": Using default 62.5% rule
