...
export class StableDataManager {
  ...
  static async getTradeFlowsData(params) {
    const startTime = Date.now()
    logDebug('STABLE: Querying comtrade reference data with quality validation', params)
    ...
    query = query
      ...
      .not('product_description', 'like', '%Test Product%')
      .not('product_description', 'like', '%agricultural and food products%')
    ...
    if (corruptedCount > 0) {
      logWarn('CORRUPTED RECORDS FILTERED OUT', {
  ...
  static detectCorruptedData(record) {
    ...
    if (record.data_source) {
      ...
      if (corruptedSources.some(source => record.data_source.includes(source))) {
        logWarn('CORRUPTED DATA SOURCE DETECTED', {
    ...
    if (record.trade_value) {
      if (record.trade_value > 10000000000 || record.trade_value < 0) {
        logWarn('UNREALISTIC TRADE VALUE DETECTED', {
    ...
    if (isCorrupted) {
      logWarn('CORRUPTED PATTERN DETECTED', {
        description: record.product_description,
...
export class DatabaseIntelligenceBridge {
  ...
  static async getAuthenticTradeFlows(params) {
    const startTime = Date.now()
    logInfo('BRIDGE: Getting authentic trade flows with corruption filtering', params)
    ...
    query = query
      ...
      .not('data_source', 'like', '%VICTORY_PUSH%')
      .not('data_source', 'like', '%TEST_PHASE%')
      .not('product_description', 'like', '%Victory Product%')
      .not('product_description', 'like', '%Test Product%')
      .not('product_description', 'like', '%base metal product %')
    ...
    if (error) {
      logError('Failed to fetch authentic trade flows', { params, error })
      return {
    ...
    const authenticFlows = data?.filter(flow => {
      ...
      const hasAuthenticDescription = flow.product_description && 
        flow.product_description.length > 10 &&
        !this.detectCorruptedData(flow)
      
      // Check for valid country codes
      const hasValidCountries = flow.reporter_country && 
        flow.partner_country &&
        flow.reporter_country.length >= 2 &&
        flow.partner_country.length >= 2
    ...
    if (corruptedCount > 0) {
      logWarn('CORRUPTED TRADE FLOWS FILTERED', {
  ...
  static async getTriangleRoutingIntelligence(params) {
    const startTime = Date.now()
    logInfo('BRIDGE: Getting triangle routing intelligence with authentic data validation', params)
