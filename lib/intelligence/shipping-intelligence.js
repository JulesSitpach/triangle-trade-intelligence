...
export class ShippingIntelligence {
  ...
  static extractSeasonalPatternsFromShippingData(shippingData) {
    ...
    if (shippingData.rates && Array.isArray(shippingData.rates)) {
      shippingData.rates.forEach((rate, index) => {
        patterns.push({
          quarter: ((index % 4) + 1),
          constraint_level: rate.amount > SHIPPING_CONFIG.capacityThresholds.high ? 'HIGH' : 
                          rate.amount > SHIPPING_CONFIG.capacityThresholds.medium ? 'MEDIUM' : 'LOW',
  ...
  static extractCarrierPerformanceMetrics(carrierData) {
    ...
    const metrics = {
      averageTransitTime: SHIPPING_CONFIG.defaultTransitTime, // Environment configurable
      reliabilityScore: SHIPPING_CONFIG.defaultReliabilityScore,   // Environment configurable
      costEfficiency: SHIPPING_CONFIG.defaultCostEfficiency,     // Environment configurable
