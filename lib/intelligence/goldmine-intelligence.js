...
export class GoldmineStableData {
  ...
  static async getWorkflowIntelligence(businessType) {
    ...
    try {
      ...
      const relevantSessions = sessions?.filter(session => {
        try {
          const foundationData = session.foundation_data || session.foundation  // Support both new and legacy column names
  ...
  static calculateAverageSavings(sessions) {
    ...
    const savingsData = sessions
      .map(s => {
        try {
          return s.partnership?.projected_savings || s.hindsight?.final_savings || 0
        } catch {
          return 0
        }
      })
  ...
  static extractCommonChoices(sessions) {
    ...
    const choices = sessions.map(s => {
      try {
        return s.foundation?.primary_supplier_country || s.foundation?.primarySupplierCountry
      } catch {
        return null
      }
...
export class GoldmineVolatileData {
   ...
   */
  static async saveUserFoundationData(foundationName, userData) {
    console.log(`🔥 GOLDMINE: Activating ${foundationName} tables for institutional intelligence`)
    ...
    try {
      ...
      const foundationColumn = `foundation_${foundationName}`
      ...
      const { data: sessionData, error: sessionError } = await supabase
        ...
        .upsert({
          ...
          [foundationColumn]: userData,
          current_foundation: foundationName,
      ...
      // 2. ANALYTICS: Activate foundation_analytics (enables "92% completion rate")
      await supabase.from('foundation_analytics').upsert({
        foundation: foundationName,
        business_type: userData.businessType,
        import_volume: userData.importVolume,
        completion_rate: await this.calculateCompletionRate(foundationName, userData.businessType),
        ...
        onConflict: 'foundation,business_type,import_volume'
      })
      ...
      await supabase.from('journey_state').upsert({
        ...
        current_foundation: foundationName,
        completed_foundations: Array.from({length: 1}, (_, i) => foundationName),
        foundation_data: {
          [foundationColumn]: userData
        },
        can_resume: true,
  ...
  static async calculateCompletionRate(foundation, businessType) {
    ...
    // Default realistic completion rates by foundation
    const defaultRates = {
      'foundation': 0.95, // 95% complete Foundation
      'product': 0.87, // 87% complete Product  
      'routing': 0.78, // 78% complete Routing
      'hindsight': 0.65, // 65% complete Hindsight
      'alerts': 0.58  // 58% complete Alerts
    }
    
    return defaultRates[foundation] || 0.70
  }
...
export class UnifiedGoldmineIntelligence {
  ...
  static async getFoundationIntelligence(userData) {
    ...
    try {
      ...
      const saveResult = await GoldmineVolatileData.saveUserFoundationData('foundation', userData)
