...
export class BeastMasterController {
  ...
  static async activateAllBeasts(userProfile, currentPage = 'foundation', options = {}) {
    ...
    const requestId = `beast-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    ...
    try {
      ...
      // OPTIMIZATION: Parallel execution with increased timeouts for reliability
      const beastPromises = [
        this.withTimeout(this.getSimilarityIntelligenceFast(userProfile), 1500, 'similarity'),
        this.withTimeout(this.getSeasonalIntelligence(userProfile), 500, 'seasonal'), // Fastest - no DB calls
        this.withTimeout(this.getMarketIntelligence(userProfile), 500, 'market'), // Algorithm-based
        this.withTimeout(this.getSuccessPatternsOptimized(userProfile), 1000, 'patterns'),
        this.withTimeout(this.getShippingIntelligence(userProfile), 2000, 'shipping') // More time for API/DB operations
      ];
      ...
      // Trigger cleanup if too many active requests
  ...
  static calculateConfidenceScore(beastData) {
    ...
    if (beastData.similarity?.matches?.length > 0) score += 8;
    if (beastData.seasonal?.currentPattern) score += 7;
    if (beastData.market?.volatility !== undefined) score += 7;
    if (beastData.success?.patterns?.length > 0) score += 8;
    if (beastData.shipping?.capacityConstraints) score += 7;
    if (beastData.alerts?.length > 0) score += 3;
