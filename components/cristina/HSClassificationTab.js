...
export default function CrisisResponseTab() {
  const [classificationRequests, setReviewRequests] = useState([]);
  const [intakeFormModal, setIntakeFormModal] = useState({
    isOpen: false,
    clientInfo: null
  });

  const [classificationModal, setReviewModal] = useState({
    isOpen: false,
    request: null,
    currentStage: 1,
    formData: {},
  ...
  const loadReviewRequests = async () => {
    try {
      ...
      if (cristinaData && cristinaData.service_requests) {
        const classificationRequests = cristinaData.service_requests.crisis_response || [];

        const enhancedRequests = classificationRequests.map(request => ({
          ...request,
  ...
  const nextReviewStage = () => {
    if (classificationModal.currentStage < 2) {
      setReviewModal({
        ...classificationModal,
        currentStage: classificationModal.currentStage + 1
      });
    }
  };

  const prevReviewStage = () => {
    if (classificationModal.currentStage > 1) {
      setReviewModal({
        ...classificationModal,
        currentStage: classificationModal.currentStage - 1
      });
  ...
  const completeReview = () => {
    console.log('Completing review for:', classificationModal.request?.company_name);
    handleUpdateStatus(classificationModal.request?.id, 'completed');
    setReviewModal({
      isOpen: false,
      request: null,
      currentStage: 1,
      ...
      <div className="tab-content">
        ...
        <table className="admin-table">
          ...
          <tbody>
            ...
            ) : classificationRequests.map(request => (
              <tr key={request.id}>
          ...
          <div className="modal-content sourcing-modal">
            ...
            <div className="verification-progress">
              <div className="progress-steps">
                <div className={`step ${classificationModal.currentStage >= 1 ? 'active' : ''}`}>1. Document Upload</div>
                <div className={`step ${classificationModal.currentStage >= 2 ? 'active' : ''}`}>2. Compliance Review</div>
                <div className={`step ${classificationModal.currentStage >= 3 ? 'active' : ''}`}>3. Review Report</div>
              </div>
            </div>

            <div className="verification-form">
              <h3>Stage {classificationModal.currentStage}: {
                classificationModal.currentStage === 1 ? 'Document Upload & Requirements' :
                classificationModal.currentStage === 2 ? 'Cristina\'s Compliance Review' :
                'Final Review Report & Recommendations'
              }</h3>

              {classificationModal.currentStage === 1 && (
              ...
              {classificationModal.currentStage === 2 && (
                <div className="document-collection-grid">
              ...
              {classificationModal.currentStage === 3 && (
                <div className="document-collection-grid">
