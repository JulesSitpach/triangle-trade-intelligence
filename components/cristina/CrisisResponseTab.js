...
function ActionPlanStage({ request, subscriberData, serviceDetails, stageData, onComplete, loading }) {
  // Cristina's crisis management input (following checklist pattern)
  ...
  return (
    <div className="workflow-stage">
      ...
      {/* Cristina's Professional Crisis Management Input */}
      <div className="professional-validation-form">
        <h4>👩‍💼 Cristina's Crisis Management Expertise</h4>
        <p className="form-helper-text">Use your logistics management experience to assess and respond to this crisis</p>

        <div className="form-group">
          <label><strong>Crisis Severity Assessment:</strong></label>
          <textarea
            className="form-input"
            rows="3"
            value={crisisSeverityAssessment}
            onChange={(e) => setCrisisSeverityAssessment(e.target.value)}
            placeholder="Based on my logistics management experience, this crisis is [Critical/High/Medium] because..."
          />
        </div>

        <div className="form-group">
          <label><strong>Immediate Actions (24-48 hours):</strong></label>
          <textarea
            className="form-input"
            rows="5"
            value={immediateActions}
            onChange={(e) => setImmediateActions(e.target.value)}
            placeholder="Hour 1: [specific action]. Hour 6: [specific action]. Day 2: [specific action]..."
          />
        </div>

        <div className="form-group">
          <label><strong>Recovery Timeline:</strong></label>
          <textarea
            className="form-input"
            rows="4"
            value={recoveryTimeline}
            onChange={(e) => setRecoveryTimeline(e.target.value)}
            placeholder="Week 1: [milestone]. Week 2: [milestone]. Full resolution: [date]"
          />
        </div>

        <div className="form-group">
          <label><strong>Risk Mitigation Strategy:</strong></label>
          <textarea
            className="form-input"
            rows="4"
            value={riskMitigation}
            onChange={(e) => setRiskMitigation(e.target.value)}
            placeholder="To prevent recurrence: [specific supply chain changes, documentation processes, vendor requirements]"
          />
        </div>
        ...
        <button
          className="btn-primary"
          onClick={async () => {
            if (!crisisSeverityAssessment || !immediateActions || !recoveryTimeline || !riskMitigation) {
              alert('⚠️ Please complete all crisis management fields');
              return;
            }

            try {
              setGeneratingReport(true);
              const response = await fetch('/api/generate-crisis-response-report', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
              ...
              if (result.success) {
                alert('✅ Crisis response report sent to triangleintel@gmail.com');
                onComplete({
                  crisis_response_completed: true,
                  cristina_crisis_management: {
                    crisis_severity_assessment: crisisSeverityAssessment,
                    immediate_actions: immediateActions,
                    recovery_timeline: recoveryTimeline,
                    risk_mitigation: riskMitigation
        ...
        >
          {generatingReport ? '⏳ Generating...' : '📧 Complete & Send Crisis Response Report'}
        </button>
      </div>
    </div>
  );
